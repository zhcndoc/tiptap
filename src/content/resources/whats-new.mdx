...
<Callout title="Beta 支持" variant="hint">
  Tiptap V3 当前处于 Beta 阶段，稳定版本预计将在 2025 年稍晚发布。请通过在 [GitHub 代码库](https://github.com/ueberdosis/tiptap/tree/next) 提交问题来支持开源版本。
</Callout>
...
在更新任何 Tiptap 或 Tiptap Pro Beta 包时，请确保所有相关包同时使用 `@next` 标签进行升级。这确保您在各个方面都安装了最新的 Beta 版本。部分更新可能会导致对等依赖冲突，因为我们当前在 Beta 阶段强制执行所有 Tiptap 包之间的严格版本对齐。
