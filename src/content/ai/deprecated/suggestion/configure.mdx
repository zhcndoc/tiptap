...
你可以在本指南中了解有关规则的更多信息：[定义规则](/ai/deprecated/suggestion/features/define-rules)
...
要了解有关建议对象应包含哪些数据的更多信息，请查看 [API 参考](/ai/deprecated/suggestion/api-reference#proofreading-suggestions)。
...
要了解如何在选择建议时显示弹出框，请按照[本指南](/ai/deprecated/suggestion/features/display-suggestions#show-a-popover-when-you-select-a-suggestion)操作。
...
默认情况下，AI 建议扩展会在编辑器就绪时自动加载建议。你可以使用 `loadOnStart` 选项禁用此行为。
...
默认情况下，AI 建议扩展会在编辑器内容更改时重新加载建议。你可以使用 `reloadOnUpdate` 选项禁用此行为。
...
你可以在[本指南](/ai/deprecated/suggestion/features/configure-when-to-load-suggestions)中了解如何配置何时加载建议。
...
如果你没有[提供自己的后端](/ai/deprecated/suggestion/custom-llms)，AI Suggestion 扩展将使用 Tiptap Content AI 云来生成建议。
...
如果你想使用自己的后端和 LLM 来生成建议，可以提供一个自定义的 `resolver` 函数。该函数应根据编辑器内容和规则返回建议数组。

你可以在[本指南](/ai/deprecated/suggestion/custom-llms)中找到一份更全面的集成你自己的后端和 LLM 的指南。
...
你可以在[本指南](/ai/deprecated/suggestion/features/provide-llm-context)中了解更多有关提供上下文的信息。
