...
<Requirements>
  ...
  <RequirementItem label="2. 集成 AI 提供商">
    在您的 [AI 设置](https://cloud.tiptap.dev/v2/cloud/ai)中配置 OpenAI，或按照
    [自定义 AI Agent 指南](/ai/deprecated/agent/custom-llms/overview)将其与其他
    AI 提供商集成。
  </RequirementItem>
  <RequirementItem label="3. 从私有注册表安装">
    要安装前端扩展，请按照 [设置指南](/guides/pro-extensions)通过认证登录到
    Tiptap 的私有 npm 注册表。
  </RequirementItem>
</Requirements>
...
<Callout title="注意" variant="info">
  本指南依赖于将 [Tiptap Cloud](https://cloud.tiptap.dev/v2/cloud/ai) 作为后端使用。如果
  您想在自己的后端中使用它，请改为遵循[自定义 AI Agent
  指南](/ai/deprecated/agent/custom-llms/overview)。
</Callout>
