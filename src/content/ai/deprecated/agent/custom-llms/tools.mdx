...
- [**客户端工具**](/ai/deprecated/agent/custom-llms/client-side-tools) 在客户端运行。它们与编辑器的内容交互。例如：一个统计文档中单词数量的工具，或者一个格式化文档的工具。所有内置工具都是客户端工具。
- [**服务端工具**](/ai/deprecated/agent/custom-llms/server-side-tools) 在服务端运行。它们与外部数据源交互。例如：一个搜索网页的工具，或者一个查询数据库的工具。
