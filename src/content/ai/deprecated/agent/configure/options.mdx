...
你可以在将 AI 代理与 Tiptap Cloud 一起使用时，为其 [定义自定义系统提示](/ai/deprecated/agent/configure/system-prompt)。
...
当启用 `autoSaveCheckpoints` 选项时，会在发送用户消息之前以及调用工具之前保存检查点。如果你想自定义检查点的保存方式和时机，可以禁用此选项并手动使用 `setCheckpoint()` 方法。有关检查点的更多信息，请参阅 [检查点指南](/ai/deprecated/agent/features/checkpoints)。
...
`reviewOptions` 选项可让您定义用于审查、接受和拒绝 AI 生成更改的工作流。更多信息请参阅 [审查 AI 生成的更改](/ai/deprecated/agent/review) 指南。
...
当 AI Agent 扩展停止运行且其 [生命周期](/ai/deprecated/agent/features/lifecycle) 结束时，会调用 `onStopRunning` 处理器。
...
这些配置选项可让你将 AI Agent 扩展与自定义 AI 代理集成，而不是使用 Tiptap Cloud 托管的 AI 代理。要了解更多信息，请阅读 [自定义 AI 代理集成指南](/ai/deprecated/agent/custom-llms/overview)。
...
`toolHandlers` 选项允许您定义 AI 代理可以调用的自定义客户端工具。此选项仅在将 AI Agent 扩展与您自己的后端一起使用时才有效。要了解如何设置自定义工具，请阅读 [自定义 AI 代理集成指南](/ai/deprecated/agent/custom-llms/overview)。
...
配置 AI Agent 扩展，使其可以[生成和理解自定义节点](/ai/deprecated/agent/configure/custom-nodes)。
