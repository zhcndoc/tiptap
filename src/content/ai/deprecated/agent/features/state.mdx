...
| -------------- | ----------------------------------- | ------------------------------------------------------------------------------------------------------- |
| `status`       | `AiAgentStatus`                     | 当前状态（loading、error 等）                                                                   |
| `loadingError` | `unknown`                           | 加载过程中发生的错误（如果有）                                                              |
| `chatMessages` | `ChatMessage[]`                     | 会话中所有消息的列表                                                                |
| `autoAccept`   | `'always' \| 'never' \| 'onlyRead'` | 当前的 [auto-accept](/ai/deprecated/agent/review#auto-accept-configuration) 设置 |
| `systemPrompt` | `string \| null`                    | 自定义 [system prompt](/ai/deprecated/agent/configure/system-prompt)                          |
...
对话的初始消息定义在 `initialMessages` 选项中。要修改消息列表，provider 包含以下方法：
