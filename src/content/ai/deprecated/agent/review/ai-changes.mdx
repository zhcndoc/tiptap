...
<Callout title="Note" variant="info">
  我们在[介绍页面](/ai/deprecated/agent/overview)提供了一个如何集成 AI 更改扩展的代码示例。
</Callout>
...
const provider = new AiAgentProvider({
  ...
  reviewOptions: {
    // 将扩展设置为 "aiChanges" 以使用 AI 更改进行审核
    extension: 'aiChanges',
...
<Callout title="注意" variant="info">
  AI 更改扩展允许用户在更改应用到文档后对其进行审阅。要在更改应用之前进行审阅，请集成 [AI
  Suggestion](/ai/deprecated/agent/review/ai-suggestion) 扩展。
</Callout>
