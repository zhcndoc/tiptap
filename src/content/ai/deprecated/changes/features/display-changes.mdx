...
若需更高级的样式，请使用 `getCustomDecorations` [配置选项](/ai/deprecated/changes/configure#custom-styles)。
...
要了解如何在选中某个变更时显示弹出框，请参考[本指南](/ai/deprecated/changes/features/display-changes#show-a-popover-when-a-change-is-selected)。
...
我们建议使用 [Floating UI](https://floating-ui.com/) 库来显示弹出层。你可以在 [演示](/ai/deprecated/changes/overview) 中查看如何实现的示例。

## 在编辑器外侧边栏显示变更

你可以从扩展的 [storage 对象](/ai/deprecated/changes/api-reference#extension-storage) 获取变更数据：
