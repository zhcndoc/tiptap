...
最常见的模式是在生成 AI 内容之前立即调用 `startTrackingAiChanges`。这使您能够捕获所有 AI 生成的更改以供审阅。例如，您可以将其与 [Basic AI Generation 扩展命令](/ai/basic/text-generation/built-in-commands) 链式调用：
...
- `getSelectedChange`: 返回当前选中的更改。当光标位于某个更改上方时，该更改会被选中。
- `getIsTrackingAiChanges`: 返回一个布尔值，表示编辑器当前是否正在跟踪 AI 更改。
- `getIsShowingAiChanges`: 返回一个布尔值，表示编辑器当前是否正在显示 AI 更改。AI 更改可通过 `setShowAiChanges` 命令[临时隐藏](/ai/deprecated/changes/features/display-changes#hide-and-show-changes)。
- `getPreviousDoc`: 返回 AI 修改之前的上一个文档。如果扩展当前未在跟踪更改，则该值为 `null`。

要了解如何使用这些方法在编辑器中显示更改，请参阅[显示 AI 更改](/ai/deprecated/changes/features/display-changes)指南。
...
您可以在 [API Reference](/ai/deprecated/changes/api-reference) 中了解有关 AI Changes 扩展命令的更多信息。
