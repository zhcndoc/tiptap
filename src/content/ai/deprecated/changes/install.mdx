...
<Callout title="已弃用" variant="warning">
  此扩展正在被 AI Toolkit 取代，后者提供了更强大的功能和现代化的 API。虽然你可以继续使用此扩展，但我们推荐在新项目中使用 AI Toolkit。
</Callout>
...
要开始跟踪更改，请调用 `startTrackingAiChanges` 命令。该命令会创建当前编辑器内容的快照，用于与 AI 生成的新内容进行比较。

```ts
editor.commands.startTrackingAiChanges()
```

调用此命令后，对编辑器内容所做的任何修改——无论是手动进行的，还是通过 [基础 AI 生成](/ai/basic/overview) 扩展进行的——都会被跟踪并显示为更改。
...
有关审阅、接受和拒绝更改的更多详细信息，请参阅 [审阅内容指南](/ai/deprecated/changes/features/review-changes)。
