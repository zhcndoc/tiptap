...
meta:
  title: REST API (旧版) | Tiptap Server AI 工具包
...
<Callout title="已弃用的端点" variant="warning">
  这些 API 端点已弃用，请参见[新的 API
  端点](/ai/ai-toolkit/api-reference/rest-api)。
</Callout>
...
更多信息请参阅 [授权指南](/ai/ai-toolkit/install#set-up-authorization)。

这些端点也接受新的 [认证](/authentication)。例外是 `stream-tool`，它要求使用新的认证方式，不接受旧版令牌。
...
- `editorContext` (`EditorContext`, 必需). 来自 `getEditorContext` 函数的编辑器上下文。请参阅[安装指南](/ai/ai-toolkit/install)。
- `tools?` (`Record<string, boolean>`, 可选)：启用或禁用单个工具。请参阅
  [工具参考](#available-tools)
  了解支持的键。
- `operationMeta?` (`string`, 可选，默认值：`""`)
- `format?` (`'json' | 'shorthand'`, 可选，默认值：`'json'`)
- `schemaAwarenessData?` (`SchemaAwarenessData`, 可选，已弃用)：请改用 `editorContext`。为向后兼容而保留支持。
...
- `toolName` (`string`, 必需)
- `input` (必需)：由 AI 模型生成的工具输入。其形状取决于
  `toolName`；请参阅[工具参考](#available-tools)。
- `editorContext` (`EditorContext`, 除非提供 `schemaAwarenessData`，否则为必需)
- `schemaAwarenessData?` (`SchemaAwarenessData`, 可选，已弃用)：请改用 `editorContext`。为向后兼容而保留支持。
- `toolConfig?` (可选)：由开发者提供而非由
  AI 模型提供的工具特定参数。请参阅[工具参考](#available-tools)。
- `format?` (`'json' | 'shorthand'`, 可选，默认值：`'json'`)
- `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/api-reference/review-options), 可选，默认值：`{ mode: 'disabled' }`)
...
- `input` (`unknown`, required): 生成的插入内容负载
...
- [审阅选项](/ai/ai-toolkit/api-reference/review-options)
