...
<Callout title="来自 AI agent 聊天机器人指南的后续内容" variant="info">
  本指南是 [AI agent 聊天机器人指南](/ai/ai-toolkit/agents/ai-agent-chatbot) 的后续内容。请先阅读该指南。
</Callout>
...
- `name`（`string`）：元素的人类可读英文名称
- `description?`（`string`）：供 AI 模型使用的元素英文说明
...
- [编辑器上下文 API 参考](/ai/ai-toolkit/api-reference/editor-context)
- [REST API 参考](/ai/ai-toolkit/api-reference/rest-api)
