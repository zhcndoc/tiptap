...
title: 模板工作流
meta:
  title: 使用 AI Toolkit 构建模板工作流 | Tiptap AI Toolkit
  description: 使用 Tiptap AI Toolkit 用 AI 生成的内容填充结构化模板。
  category: AI Toolkit
---
...
如果你的后端使用的不是 TypeScript，而是其他编程语言，请参见 [本指南](/ai/ai-toolkit/client/advanced-guides/non-typescript-backends)。
...
要将模板结果插入到特定区域，而不是替换整个文档，请设置 `position` 选项。它可以接受一个位置或一个 [范围](/ai/ai-toolkit/client/advanced-guides/concepts#range)。
...
默认情况下，`templateWorkflow` 方法填充模板后会移除所有 `_templateSlot` 属性。将 `preserveSlotAttr` 设置为 `true`，即可在填充后的节点上保留这些属性：
...
启用 `preserveSlotAttr` 后，每个填充后的节点都会保留其 `_templateSlot` 属性。这使你能够：

- **识别 AI 填充的内容**：检查文档中的哪些部分由 AI 生成。
- **重新填充模板字段**：将填充后的文档用作新模板，以重新生成特定部分。
- **保留字段元数据**：稍后从填充后的文档中读取 `_templateFieldMetadata`。
...
- `templateWorkflow` 方法的 [API 参考](/ai/ai-toolkit/client/api-reference/workflows#templateworkflow)
- `createHtmlTemplate` 方法的 [API 参考](/ai/ai-toolkit/client/api-reference/workflows#createhtmltemplate)
- `createTemplateWorkflow` 工具的 [API 参考](/ai/ai-toolkit/client/api-reference/workflows#createtemplateworkflow-server-side-utility)。
