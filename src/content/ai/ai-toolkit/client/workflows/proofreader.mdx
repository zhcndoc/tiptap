...
title: 校对工作流程
meta:
  title: 使用 AI Toolkit 构建校对器 | Tiptap AI Toolkit
  description: 使用 Tiptap AI Toolkit 构建一个校对器，用于检测并纠正文档中的拼写错误。
  category: AI 工具包
---
...
<Callout title="专业版包" variant="hint">
  AI 工具包是一个专业版包。安装前，请按照 [私有注册表指南](/guides/pro-extensions) 配置私有 NPM 注册表的访问权限。
</Callout>
...
创建一个 API 端点，使用 [Vercel AI SDK](https://ai-sdk.dev/) 调用 OpenAI 模型。如果你的后端不是 TypeScript，请参阅 [非 TypeScript 后端](/content-ai/capabilities/ai-toolkit/advanced-guides/non-typescript-backends)。

如果你的后端使用的是除 TypeScript 以外的其他编程语言，请参阅 [本指南](/ai/ai-toolkit/client/advanced-guides/non-typescript-backends)。
...
要校对文档的一个子部分，请设置 `range` 选项。此参数接受一个 [Range](/ai/ai-toolkit/client/advanced-guides/concepts#range)，表示将要校对的文档区域。
...
对于大型文档，您可以使用 [`tiptapReadChunks`](/ai/ai-toolkit/client/api-reference/read-the-document#tiptapreadchunks) 方法将文档拆分为多个块并并行处理。每个块都包含一个 [Range](/ai/ai-toolkit/client/advanced-guides/concepts#range)，表示该块所对应的区域。
...
- 使用 [**Tracked Changes**](/tracked-changes/getting-started/overview) 扩展来渲染审阅界面。更改会作为文档的一部分持久保存，并对其他用户可见。
...
请参阅 [review changes](/ai/ai-toolkit/client/agents/review-changes) 指南，了解更多关于审阅界面的信息。

请查看 [AI Toolkit 演示](https://github.com/ueberdosis/ai-toolkit-demos)，了解如何将 AI Toolkit 工作流程与审阅界面结合使用的示例。

## 相关指南

- [`tiptapRead`](/ai/ai-toolkit/client/api-reference/read-the-document#tiptapread) 方法的 [API 参考]
- [`tiptapReadChunks`](/ai/ai-toolkit/client/api-reference/read-the-document#tiptapreadchunks) 方法的 [API 参考]
- [`proofreaderWorkflow`](/ai/ai-toolkit/client/api-reference/workflows#proofreaderworkflow) 方法的 [API 参考]
