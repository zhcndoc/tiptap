...
title: 评论工作流
meta:
  title: 评论工作流 | Tiptap AI Toolkit
  description: 使用 Tiptap AI Toolkit 构建一个工作流，用于管理文档中的评论和线程。
  category: AI Toolkit
---
...
创建一个使用 [Vercel AI SDK](https://ai-sdk.dev/) 调用 Anthropic 模型的 API 端点。如果你的后端不是 TypeScript，请参阅[非 TypeScript 后端](/content-ai/capabilities/ai-toolkit/advanced-guides/non-typescript-backends)。
...
## Commenting on a Section of the Document

To add comments only to a subsection of the document, set the `range` option on `tiptapRead`. This parameter accepts a [Range](/ai/ai-toolkit/client/advanced-guides/concepts#range) containing the area to read.

For example, to comment only on the selected content:

```ts
const range = editor.state.selection

// Only read the selected part of the document
...
For large documents, you can use the [`tiptapReadChunks`](/ai/ai-toolkit/client/api-reference/read-the-document#tiptapreadchunks) method to split the document into multiple chunks and process them in parallel. Each chunk contains a [Range](/ai/ai-toolkit/client/advanced-guides/concepts#range) representing the area of that chunk.

## 相关指南

- [`tiptapRead`](/ai/ai-toolkit/client/api-reference/read-the-document#tiptapread) 方法的 [API 参考](/ai/ai-toolkit/client/api-reference/read-the-document#tiptapread)
- [`getThreads`](/ai/ai-toolkit/client/api-reference/workflows#getthreads) 方法的 [API 参考](/ai/ai-toolkit/client/api-reference/workflows#getthreads)
- [`editThreadsWorkflow`](/ai/ai-toolkit/client/api-reference/workflows#editthreadsworkflow) 方法的 [API 参考](/ai/ai-toolkit/client/api-reference/workflows#editthreadsworkflow)
- [评论扩展](/comments/getting-started/overview)：了解更多关于评论扩展的信息。
