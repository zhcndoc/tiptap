...
title: 流式传输
meta:
  title: 流式传输 | Tiptap AI 工具包
  description: 为你的 AI 智能体聊天机器人添加实时流式传输，以便在变化发生时立即看到更新。
  category: AI 工具包
---
...
<Callout title="来自 AI 代理聊天机器人指南的延续" variant="info">
  本指南接续 [AI 代理聊天机器人
  指南](/ai/ai-toolkit/client/agents/ai-agent-chatbot)。请先阅读它。
</Callout>
...
要将我们在[上一份指南](/ai/ai-toolkit/client/agents/ai-agent-chatbot)中构建的 AI 代理聊天机器人添加流式传输功能，我们需要将 `executeTool` 方法替换为 `streamTool` 方法。
...
要在我们在[上一份指南](/ai/ai-toolkit/client/agents/ai-agent-chatbot)中构建的 AI 代理聊天机器人中实现此流程，请按照以下步骤操作：
...
Here is the complete updated component including tool streaming:
...
export default function Page() {
  const editor = useEditor({
    ...
    content: `<h1>AI Agent Demo</h1><p>Please ask AI to improve this content.</p>`,
  })
  ...
  const { messages, sendMessage, addtoolOutput } = useChat({
    ...
    async onToolCall({ toolCall }) {
      ...
      // When tool streaming is complete, we need to apply the tool call to the document
      ...
      const result = toolkit.streamTool({
        ...
        toolName,
        input,
        // This parameter indicates that tool streaming has finished
        hasFinished: true,
      })
  ...
  const [input, setInput] = useState(
    'Insert a long story about Tiptap with 10 paragraphs at the end of the document',
  )

  // While tool streaming is in progress, we need to update the document as the tool input changes
  useEffect(() => {
    if (!editor) return
    ...
    // Find the last tool just called by AI
    const toolCallParts = lastMessage.parts.filter((p) => p.type.startsWith('tool-')) ?? []
    const lastToolCall = toolCallParts[toolCallParts.length - 1]
    if (!lastToolCall) return
    ...
    // Apply the tool call to the document during streaming
    const toolkit = getAiToolkit(editor)
    toolkit.streamTool({
      toolCallId: part.toolCallId,
      toolName,
      input: part.input,
      // This parameter indicates that tool streaming has not yet finished
      hasFinished: false,
    })
  }, [addtoolOutput, editor, messages])
...
请参阅 [AI Caret 指南](/ai/ai-toolkit/client/advanced-guides/ai-caret) 了解配置选项和 CSS 样式。
...
请参阅 [审阅更改](/ai/ai-toolkit/client/agents/review-changes) 指南，了解有关审阅界面的更多信息。

有关如何将 AI Toolkit 流式传输与审阅界面结合使用的示例，请参阅 [AI Toolkit 演示](https://github.com/ueberdosis/ai-toolkit-demos)。

## 下一步

- [查看更改指南](/ai/ai-toolkit/client/agents/review-changes): 让用户在更改应用之前预览并批准更改。
