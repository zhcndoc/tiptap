...
meta:
  title: 样式建议指南 | Tiptap AI Toolkit
  description: 将样式应用于建议，使用 AI Toolkit。
  category: AI 工具包
---

当 AI 对文档进行更改时，你可以在编辑器中将它们显示为[建议](/ai/ai-toolkit/client/api-reference/suggestions)。在本指南中，你将学习如何通过修改建议的样式并在其中显示自定义元素（例如菜单）来定制它们的外观。
...
在 [预览模式](/ai/ai-toolkit/client/agents/review-changes#two-types-of-suggestions) 中使用这些样式，用于在更改插入文档之前进行预览。
...
在 [审阅模式](/ai/ai-toolkit/client/agents/review-changes#two-types-of-suggestions) 中使用这些样式，用于在文档中插入更改后进行审阅。

在显示由 [比较文档](/ai/ai-toolkit/client/advanced-guides/compare-documents) 功能生成的建议时，也请使用这些样式。
...
import { isAiToolkitSuggestionNodeView } from '@tiptap-pro/client-ai-toolkit'
...
function MathBlockNodeView({ decorations }) {
  ...
  return (
    <NodeViewWrapper contentEditable={!isAiToolkitSuggestion}>
      {!isAiToolkitSuggestion ? <button>编辑公式</button> : null}
      {/* 在此处渲染节点视图内容。 */}
    </NodeViewWrapper>
  )
}
...
然后，在 `displayOptions.renderDecorations` 选项中，创建一个带有 HTML 元素的小部件装饰，该元素将用作 React 门户的入口点。
...
```ts
import { getNextWord, getPreviousWord } from '@tiptap-pro/client-ai-toolkit'
...
- 在 [API 参考](/ai/ai-toolkit/client/api-reference/suggestions) 中了解有关建议的更多信息。请参阅 `displayOptions` 参数以了解所有自定义选项。
