...
title: 建议
meta:
  title: 使用建议审阅 AI 更改 | Tiptap AI Toolkit
  description: 使用可自定义渲染的建议预览并批准 AI 插入的更改。
  category: AI 工具包
---
...
<Callout title="从 AI 代理聊天机器人指南继续" variant="info">
  本指南是 [AI 代理聊天机器人指南](/ai/ai-toolkit/client/agents/ai-agent-chatbot) 的续篇。请先阅读前者。
</Callout>
...
执行工具后，多个**建议**会显示在编辑器内。每条建议都代表 AI 模型对文档所做的更改。
      ...
      <button
        onClick={() => {
          ...
          const rejectionMessage =
            '用户对您所做的某些更改已被拒绝。请询问用户原因，以及您可以如何改进这些更改。'
...
在 `executeTool` 方法的 [API 参考](/ai/ai-toolkit/client/api-reference/execute-tool) 中查看 `reviewOptions` 参数的完整参考。

你可以将本指南与 [流式传输](/ai/ai-toolkit/client/agents/streaming) 结合使用，以便在审核建议时流式处理操作，并保持助手的响应能力。

### 自定义建议的显示方式

在审核更改时，AI Toolkit 会将更改的预览显示为 [建议](/ai/ai-toolkit/client/api-reference/suggestions)。

你可以通过设置 `reviewOptions.displayOptions` 参数来自定义这些建议的显示方式。你甚至可以在其中 [渲染 React 组件](/ai/ai-toolkit/client/agents/review-changes/style-suggestions)。
