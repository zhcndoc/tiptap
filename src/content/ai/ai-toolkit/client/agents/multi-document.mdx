...
title: 多文档 AI 代理
meta:
  title: 多文档 AI 代理 | Tiptap AI 工具包
  description: 为你的 AI 代理聊天机器人添加多文档支持，以编辑多个 Tiptap 文档。
  category: AI 工具包
---
...
<Callout title="AI 代理聊天机器人指南的变体" variant="info">
  本指南是[AI 代理聊天机器人指南](/ai/ai-toolkit/client/agents/ai-agent-chatbot)的一个变体。请先阅读它。
</Callout>
...
在本指南中，我们将创建一个可以编辑多个文档的 AI 聊天机器人。我们将通过创建自定义工具来创建、删除和切换文档。然后，将它们与 Tiptap AI 工具包中的工具结合起来，使 AI 能够编辑文档内容。
...
<Callout title="专业版包" variant="hint">
  AI Toolkit 是专业版包。安装之前，请按照[私有注册表指南](/guides/pro-reextensions)设置对私有 NPM 注册表的访问权限。
</Callout>
...
我们将定义一个 API 端点，使用 [Vercel AI SDK](https://ai-sdk.dev/) 调用 OpenAI 模型。如果您的后端不是 TypeScript，请参阅[非 TypeScript 后端指南](/content-ai/capabilities/ai-toolkit/advanced-guides/non-typescript-backends)。

如果您的后端使用的是除 TypeScript 之外的其他编程语言，请参阅[本指南](/ai/ai-toolkit/client/advanced-guides/non-typescript-backends)。

包含 Tiptap AI Toolkit 的 [工具定义](/ai/ai-toolkit/client/agents/tools/ai-sdk)（`...toolDefinitions()`），并将它们与下面定义的自定义工具组合起来：
    ...
    tools: {
      ...
      createDocument: tool({
        ...
        inputSchema: z.object({
          ...
          content: z.string().describe('文档的 HTML 内容'),
        }),
      }),
      listDocuments: tool({
        description:
...
const initialDocuments: Document[] = [
  {
    name: '文档 1',
    content: '<h1>文档 1</h1><p>这是第一个文档的内容。</p>',
  },
]
...
export default function Page() {
  ...
  // 活动文档名称
  const [activeDocumentName, setActiveDocumentName] = useState('文档 1')
...
- 让您的用户通过 [更改审阅指南](/ai/ai-toolkit/client/agents/review-changes) 审阅 AI 生成的更改
- 使用 [流式传输指南](/ai/ai-toolkit/client/agents/streaming) 添加实时工具流，以便查看更改如何发生
