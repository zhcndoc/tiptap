...
meta:
  title: AI SDK - AI 代理工具 | Tiptap AI 工具包
  description: 使用 Vercel 的 AI SDK 工具定义，让 AI 代理能够读取和编辑 Tiptap 文档。
  category: AI 工具包
---

import { Callout } from '@/components/ui/Callout'

`@tiptap-pro/client-ai-toolkit-ai-sdk` 包提供了工具定义，你可以将其添加到使用 [Vercel 的 AI SDK](https://ai-sdk.dev/) 构建的 AI 代理中。

模型生成的工具调用随后可以使用 [`executeTool` 方法](/ai/ai-toolkit/client/api-reference/execute-tool) 来执行。
...
```bash
npm install @tiptap-pro/client-ai-toolkit-ai-sdk
```
...
<Callout title="Zod 3 版本的类型错误" variant="warning">
  如果你的应用使用了 [Zod 验证库](https://zod.dev/) 的第 3 版，在将工具定义传递给 `generateText` 或 `streamText` 函数的 `tools` 参数时，可能会出现类型错误。你可以通过升级到 Zod 4，或将工具定义类型转换为 AI SDK 的 `ToolSet` 类型来修复此问题。
</Callout>
...
一个包含已启用工具定义的对象，可与 [Vercel AI SDK](https://ai-sdk.dev/) 的工具调用系统一起使用。要查看完整的工具列表，请参阅 [可用工具](/ai/ai-toolkit/client/agents/tools) 页面。
