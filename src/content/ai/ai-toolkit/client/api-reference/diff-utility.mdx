...
title: Diff 实用工具
meta:
  title: Diff 实用工具 | Tiptap AI Toolkit
...
要逐步了解比较文档的流程，请参阅[比较文档指南](/ai/ai-toolkit/client/advanced-guides/compare-documents)。
...
- `otherDoc?` (`Node`): 要进行比较的文档。它是一个 [ProseMirror Node 对象](https://prosemirror.net/docs/ref/#model.Node)。如果省略，则默认为当前文档的快照
- `displayOptions?` (`DisplayOptions`): 控制差异如何作为建议呈现。[查看可用选项](/ai/ai-toolkit/client/api-reference/suggestions#the-suggestion-object)。
- `debounceTimeout?` (`number`): 在重新计算差异前等待的毫秒数。较小的值会让应用看起来更响应，但在大型文档中可能会导致性能问题。默认值：`500`
- `diffUtilityOptions?` ([`ExternalDiffUtilityOptions`](/ai/ai-toolkit/client/api-reference/diff-utility#parameters-diffutilityoptions)): 用于通过 diff 工具比较文档的选项。[查看可用选项](/ai/ai-toolkit/client/api-reference/diff-utility#parameters-diffutilityoptions)。
...
请参阅 [比较文档指南](/ai/ai-toolkit/client/advanced-guides/compare-documents) 以查看使用示例。
