...
- [插入内容](/ai/ai-toolkit/client/workflows/insert-content).
- [校对](/ai/ai-toolkit/client/workflows/proofreader).
- [Tiptap 编辑](/ai/ai-toolkit/client/workflows/tiptap-edit).
- [评论](/ai/ai-toolkit/client/workflows/comments).
- [模板](/ai/ai-toolkit/client/workflows/template).
...
- `options` (`ProofreaderWorkflowOptions`): 配置选项
  - `hasFinished` (`boolean`): 流式传输是否已结束。在操作仍在流式传输时传入 `false`，最终调用时传入 `true`。
  ...
  - `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/client/api-reference/review-options)): 控制预览/审阅行为。[查看可用选项](/ai/ai-toolkit/client/api-reference/review-options)。
    - 在此方法中，`diffUtilityOptions.groupInlineChanges` 默认值为 `2` 而不是 `1`，因此默认情况下编辑会显示为行内更改。
    - 当安装了 [Tracked Changes](/tracked-changes/getting-started/overview) 扩展时，支持 `mode: 'trackedChanges'`。
  - `range?` (`Range`): 要应用校对操作的范围
    - `from` (`number`): 起始位置
    - `to` (`number`): 结束位置
...
创建一个预设的文档编辑工作流配置。返回系统提示和用于验证 AI 输出的模式。

该工作流期望用户的输入为 JSON 对象，包含：
...
- `options` (`TiptapEditWorkflowOptions`): 配置选项
  ...
  - `operations` (`PartialTiptapEditOperation[]`): 编辑操作数组。每个操作是一个对象，包含：
    ...
    - `target`: 节点的 6 字符哈希值（来自 `tiptapRead`），或者整个文档使用 `'doc'`
    - `content`: 要插入内容的 HTML 字符串
  - `workflowId` (`unknown`): 此工作流运行的唯一 ID
  - `tiptapReadResult` (`TiptapReadResult`): `tiptapRead` 返回的结果，用于在应用编辑操作时进行过时读取检测。
  - `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/client/api-reference/review-options)): 控制预览/审阅行为。
    - 当安装了 [Tracked Changes](/tracked-changes/getting-started/overview) 扩展时，支持 `mode: 'trackedChanges'`。
  - `streamingOptions?` (`StreamingOptions`): 控制如何应用流式传输的操作。
    - `disableTypingEffect?` (`boolean`): 在应用流式传输的操作时禁用打字效果。
  - `tiptapEditHooks?` (`TiptapEditHooks`): 用于拦截和修改操作的钩子。详情请参见 [Tiptap Edit hooks 指南](/ai/ai-toolkit/client/advanced-guides/tiptap-edit-hooks)。
    - `beforeOperation?` (`(context: BeforeOperationContext) => BeforeOperationResult`): 在应用每个操作之前调用。返回 `{ action: 'accept' }` 以应用该操作（可选择覆盖 `fragment` 或 `operationType`），或返回 `{ action: 'reject', error }` 以跳过它。
...
- `options` (`TemplateWorkflowOptions`): 配置选项
  - `template` (`Record<string, any>`): Tiptap JSON 格式的模板（一个 doc 节点或 fragment 数组）
  - `values` (`TemplateValues`): 用于填充模板的 AI 生成值。键对应模板属性值。
  - `position?` (`Range | number`): 填充后的模板插入位置。默认：整个文档范围。
  - `hasFinished?` (`boolean`): 流式传输是否已完成。省略时，该方法以非流式模式运行。默认：`true`
  - `workflowId?` (`unknown`): 本次工作流运行的唯一 ID。提供后，将启用流式模式，此时方法会在多次调用之间跟踪插入范围，并逐步替换内容。
  - `preserveSlotAttr?` (`boolean`): 当为 `true` 时，填充后会保留节点上的 `_templateSlot` 属性。这有助于识别 AI 填充的内容，并在之后重新填充模板字段。默认：`false`
  - `reviewOptions?` ([`ReviewOptions`](/ai/ai-toolkit/client/api-reference/review-options)): 控制预览/审核行为
...
- 带有小写模板属性的 HTML `string`（`_templateslot`, `_templateif`, `_templateattributes`, `_templatefieldmetadata`）
