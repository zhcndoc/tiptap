...
<Callout title="使用自定义节点？" variant="info">
  ...
  `addJsonSchemaAwareness` 进行配置。请参阅 [自定义节点
...
// 通过密钥生成 JWT 令牌
// 包含文档服务器凭据，方便 Server AI 工具包自动获取并保存 Tiptap 云端文档。
const JWT_TOKEN = jwt.sign(
  {
    // 文档服务器凭据，用于 Tiptap 云端文档
...
<Callout title="可选：直接提供文档" variant="info">
  如果您使用自己的存储管理文档，而非 Tiptap 云，您可以省略 JWT 中的 `experimental_document_server_id` 和 `experimental_document_server_management_api_secret` 声明，直接通过 API 请求中的 `document` 字段传递文档。详见 [REST API 参考](/content-ai/capabilities/server-ai-toolkit/api-reference/rest-api)。
</Callout>
