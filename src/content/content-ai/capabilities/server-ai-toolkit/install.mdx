...
<Callout title="Tiptap 云端文档" variant="info">
  若想让 AI 服务器自动从 Tiptap 文档服务器获取和保存文档，
  请在您的 JWT 中包含 `experimental_document_app_id` 和 `experimental_document_secret` 声明。
  详见[评论指南](/content-ai/capabilities/server-ai-toolkit/agents/comments)。
</Callout>
