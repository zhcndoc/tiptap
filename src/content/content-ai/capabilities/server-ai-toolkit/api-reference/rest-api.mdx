...
**使用 Tiptap Cloud 文档（推荐）** — 在请求体中包含带有 `documentId` 的 `experimental_documentOptions`。服务器将自动从 Tiptap Document Server 获取文档、处理文档，并保存回更改。此方式需在您的 JWT 声明中包含 Document Server 凭据（见上文【Document Server 凭据】）。

**直接传递文档** — 在请求体中包含 `document` 字段，传入您的 Tiptap JSON 文档。服务器处理该文档并在响应中返回修改后的文档。

您必须提供且仅提供其一：要么 `experimental_documentOptions`，要么 `document`。提供二者或都不提供将导致验证错误。
...
**使用 Tiptap Cloud 文档（推荐）** — JWT 必须包含 `experimental_document_app_id` 和 `experimental_document_secret` 声明：
...
- `toolName` (`string`, required): 要执行的工具名称。必须是以下之一：
  - `"tiptapRead"`：使用高效格式读取文档部分内容
  - `"tiptapEdit"`：使用高效格式编辑文档
  - `"getThreads"`：获取文档中所有线程和评论
  - `"editThreads"`：对线程和评论执行操作
- `input` (`any`, required): 工具的输入参数，依据工具的输入 schema 定义。输入格式针对高效读取和编辑操作进行了优化。
- `experimental_documentOptions` (`object`, conditional): 用于引用 Tiptap Cloud 文档的 ID。服务器会自动获取并保存文档。与 `document` 互斥。
  - `documentId` (`string`, required): Document Server 上文档的 ID
  - `userId` (`string`, optional): 用于标识线程/评论作者的用户 ID
- `document` (`object`, conditional): 要操作的 Tiptap JSON 文档。当提供此字段时，须不包含 `experimental_documentOptions`。
- `experimental_commentsOptions` (`object`, optional): 线程/评论操作的元数据。
  - `threadData` (`Record<string, any>`, optional): 附加到新线程的元数据
  - `commentData` (`Record<string, any>`, optional): 附加到新评论的元数据
- `schemaAwarenessData` (`SchemaAwarenessData`, required): 从 `getSchemaAwarenessData` 获取的 schema 认知数据
- `chunkSize?` (`number`, optional): 读取操作的块大小。默认值：`32000`
- `tiptapEditSelectableNodes?` (`string[]`, optional): tiptapEdit 操作的自定义可选节点类型
- `format?` (`string`, optional): AI 读取和编辑文档所使用的格式。必须为以下之一：
  - `"json"`（默认）：标准 JSON 格式
  - `"shorthand"`：Tiptap 简写格式，相较标准 JSON 可将 AI 令牌消耗降低最多 80%
