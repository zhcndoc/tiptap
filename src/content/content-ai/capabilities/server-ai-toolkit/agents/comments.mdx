...
title: Server 评论
meta:
  title: Server 评论 | Tiptap 服务器 AI 工具包
...
- `experimental_document_server_id` (`string`): Tiptap Cloud 文档服务器 ID  
- `experimental_document_server_management_api_secret` (`string`): 文档服务器的管理员 API 密钥
...
- `threadData` (`Record<string, unknown>`, optional): 当由 `editThreads` 工具创建新线程时附加到新线程的元数据。
- `commentData` (`Record<string, unknown>`, optional): 当由 `editThreads` 工具创建新评论时附加到新评论的元数据。
