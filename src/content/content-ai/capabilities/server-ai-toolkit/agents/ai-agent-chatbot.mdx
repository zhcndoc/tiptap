...
_Schema 识别_ 是 Server AI Toolkit 让你的 LLM 理解文档结构的能力。使用此函数，你可以生成该识别内容的格式化字符串，包含在提示中以向模型描述文档。
...
<Callout title="在聊天轮次之间持久化会话" variant="info">
  将最新的 `sessionId` 存储在对话历史中，并在后续的工具调用中重用它。这样服务器就能拒绝过时的编辑，而不会覆盖更新的用户更改。
</Callout>

可以使用一个小型辅助函数，例如 [`lib/server-ai-toolkit/session-id.ts`](https://github.com/ueberdosis/ai-toolkit-demos/blob/main/src/lib/server-ai-toolkit/session-id.ts)，
从对话历史中读取最新的 `sessionId`。
