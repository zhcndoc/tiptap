...
你还需要一个 OpenAI API 密钥；如果使用 Tiptap Cloud 协作，还需要为文档服务器配置 `TIPTAP_CLOUD_SECRET`。请将这些变量与 [授权指南](/content-ai/capabilities/server-ai-toolkit/install#set-up-authorization) 中的变量一起添加到你的 `.env` 文件中：
...
你可以不用 Tiptap Cloud 文档，而是在 execute-tool 请求体中传入一个 `document` 字段（而不是 `experimental_documentOptions`），由你自己提供并管理文档。
...
更多信息请参阅 [REST API 参考](/content-ai/capabilities/server-ai-toolkit/api-reference/rest-api) 中关于 `document` 字段的说明。
