...
- **TIPTAP_CLOUD_APP_ID**：用于协作和文档管理的 Tiptap Cloud 应用 ID。
- **TIPTAP_CLOUD_DOCUMENT_MANAGEMENT_API_SECRET**：用于访问 Tiptap Cloud 文档管理 API 的 REST API 密钥。
...
此函数通过 Server AI Toolkit API 执行指定工具。它将工具名称、输入参数、当前文档状态和 schema 识别数据发送给 API，API 处理工具执行并返回结果，以及如果工具修改了文档，返回更新后的文档。
...
export async function getCollabConfig(
  ...
  if (!TIPTAP_CLOUD_APP_ID) {
    throw new Error('TIPTAP_CLOUD_APP_ID 环境变量未设置')
  }
