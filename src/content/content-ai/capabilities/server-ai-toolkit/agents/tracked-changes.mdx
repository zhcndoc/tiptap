...
要将服务端 AI 编辑显示为追踪变更，请在 `POST /v4/ai/toolkit/execute-tool` 端点上配置 `reviewOptions` 参数。在调用 `tiptapRead` 和 `tiptapEdit` 工具时，将审阅模式设置为 `{ mode: 'trackedChanges' }`。有关所有可用设置，请参阅 [review options 参考](/content-ai/capabilities/server-ai-toolkit/api-reference/review-options)。
...
- 了解更多关于 [审阅选项](/content-ai/capabilities/server-ai-toolkit/api-reference/review-options)
