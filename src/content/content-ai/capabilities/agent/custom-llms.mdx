...
<Callout title="定制 LLM 演示" variant="info">
  我们为我们的增长和企业客户提供详细的定制 LLM
  演示。它包括客户端和服务器代码，以及运行和部署的说明。请查看
  [计划和定价](https://tiptap.dev/pricing)。
</Callout>
...
适配器用于在 AI Agent 的内部格式与不同 LLM 供应商及 AI Agent 框架所使用的格式之间进行转换。在客户端配置 AI Agent 供应商时需提供相应适配器。

```ts
import { AiAgentProvider, openAiChatCompletionsAdapter } from '@tiptap-pro/extension-ai-agent'
...
`@tiptap-pro/extension-ai-agent` 包含以下内置适配器：

| Adapter                        | Provider                                                                                            |
| ------------------------------ | --------------------------------------------------------------------------------------------------- |
| `openaiChatCompletionsAdapter` | OpenAI API [聊天完成 API](https://platform.openai.com/docs/api-reference/chat?lang=node.js)（默认） |
| `openaiResponsesAdapter`       | OpenAI API [响应 API](https://platform.openai.com/docs/api-reference/responses?lang=javascript)     |
| `anthropicMessagesAdapter`     | Anthropic Claude [消息 API](https://docs.anthropic.com/en/api/messages)                             |
| `vercelAiSdkAdapter`           | [Vercel AI SDK](https://ai-sdk.dev)                                                                 |
...
| **Writing**   | `apply_diff`, `replace_document`                             |
...
export const customSearchTool = (): AiAgentTool => ({
  ...
  // 系统提示的可选附加规则
  systemPromptRules: ['当需要查找文档中特定内容时使用此工具', '搜索不区分大小写'],
})
```
