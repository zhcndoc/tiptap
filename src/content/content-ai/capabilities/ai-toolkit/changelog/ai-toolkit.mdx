...
- **破坏性变更**：移除 `getText`、`getHtml`、`getJson`、`getTextSelection`、`getHtmlSelection` 和 `getJsonSelection` 方法。请改用 Tiptap 编辑器 API 或 `getTextRange`、`getHtmlRange`、`getJsonRange` 等方法。
- **破坏性变更**：简化 `InsertPosition` 类型，由 8 个选项的联合类型改为 `Range | number`。位置参数现在接受数字偏移或 `Range` 对象。默认为整个文档范围。
- **破坏性变更**：移除 `setHtmlSuggestions` 方法，请改用校对工作流（Proofreader Workflow）。
- **破坏性变更**：移除 `smartInline` 和 `smartInlineV2` 差异模式，请改用 `smart` 模式。
- **破坏性变更**：简化 `setSuggestions` 和 `addSuggestions`，直接接受 `Suggestion[]`，不再支持 `SuggestionsFormat[]`。不再支持 `compareDocuments` 格式。请改用新的 `setSuggestionsFromDiff` 方法。
- **破坏性变更**：简化 `ReplacementOption`，由判别联合类型（`SliceReplacementOption | TextReplacementOption`）改为单一接口，包含 `content` 属性（类型为 `string | Slice`）。移除 `type`、`addSlice` 和 `addText` 属性。

### 小幅更新

- 新增 `setSuggestionsFromDiff` 方法，可比较两个文档并根据差异设置建议，替代之前 `setSuggestions` 中的 `compareDocuments` 格式。

## 3.0.0-alpha.81

### 修补更新

- 修复自定义节点视图嵌套于容器节点例如引用块或列表时无法渲染的问题。

## 3.0.0-alpha.80

### 修补更新

- 修复替换子更改标记跨节点边界时错误显示的问题。
- 修正 `simplifyChanges` 误将改动扩展至单词边界之外的问题。现改动相邻非字母字符（如空格、标点）不再被纳入改动范围。计算单词边界时，修订为同时检测原始和修改后文档内容。
...
- 修复用户修改文档后，AI 无法定位用户修改内容下方任何内容，导致必须重新读取文档的问题。

## 3.0.0-alpha.76

### 修补更新

- 修复 `offsetChanges` 在映射改动时未保留 `isInlineGroup` 属性，导致内联组信息丢失的问题。

## 3.0.0-alpha.75

### 重大更新

- **破坏性变更**：新增 `smart` 差异模式，设为默认。此模式智能适应块级和内联内容。改动绝不跨块边界，适合结构化文档修改，生成清晰差异。
- **破坏性变更**：重命名 `detailed` 差异模式为 `inline`。
- **破坏性变更**：在 `ReviewOptions` 中用布尔类型 `useDiffUtility` 取代 `diffMode` 属性。设置 `useDiffUtility: true` 取代 `diffMode: 'detailed'`，`useDiffUtility: false` 取代 `diffMode: 'full'`。
- **破坏性变更**：默认 `changeMergeDistance` 设为 `10`。
- **破坏性变更**：重命名 `displayOptions.showAsDiff` 为 `displayOptions.showReplacement`。
- **破坏性变更**：移除 `displayOptions.diffPosition`。
- **破坏性变更**：重命名 `displayOptions.diffAttributes` 为 `displayOptions.replacementAttributes`。
- **破坏性变更**：重命名类型 `DiffUtilityConfig` 为 `ExternalDiffUtilityOptions`。
- **破坏性变更**：所有方法和选项中把属性 `diffUtilityConfig` 重命名为 `diffUtilityOptions`。

### 弃用提示

- 弃用 `smartInline` 和 `smartInlineV2` 差异模式，推荐使用 `smart` 模式。

### 小幅更新

- 提升内联差异模式的性能。
- 修正差异工具中新增内容和删除内容均以空格结尾的 Bug。
- 在 `diffUtility` 和 `diffUtilityOptions` 中新增 `groupInlineChanges`，用于控制内联编辑何时分组为一个建议。
- 在 `displayOptions` 中新增 `showSubChanges`、`subChangeAttributes` 和 `replacementSubChangeAttributes`，用于自定义分组内联改动的展示。
- 在 `Suggestion` 类型中新增 `isInlineGroup` 字段，表示分组的内联改动。
- 在 `proofreaderWorkflow` 中，默认不对内联改动分组。需要时可通过 `reviewOptions.diffUtilityOptions.groupInlineChanges` 覆盖。

## 3.0.0-alpha.74

### 修复更新

- 修复在协作文档中其他用户删除建议所在内容时，建议未被移除的问题。
...
- 新增 `createHtmlTemplate` 工具，将 Tiptap JSON 模板转换成 HTML。

## 3.0.0-alpha.72

### 修复更新

- 修复 ES2025 迭代器辅助函数（如 `Map.entries().filter()`、`.map()`）在 Safari < 18.4、Firefox < 131 及其他不支持迭代器辅助函数的浏览器中导致 `TypeError` 的兼容性问题。

## 3.0.0-alpha.71

### 修复更新

- 修复建议范围内的空段落未被视觉标记的问题。

## 3.0.0-alpha.70

### 小幅更新

- 为 `toolDefinitions()` 和 `tiptapEditTool()` 新增 `operationMeta` 选项。启用后，AI 每次编辑操作会包含 `meta` 字段，说明变更原因。
...
- 启用 `simplifyChanges` 时，单字符词内编辑会扩展至整个单词边界。
- 支持单字符的插入、删除和替换，使差异更易阅读。
...
- 使用 Tiptap 位置工具进行位置映射，新版本要求 Tiptap 版本 ≥ 3.12.0。
- 全新流处理实现，允许 AI 流式期间其他用户编辑文档。影响操作包括：
  - `tiptapEdit` 工具
  - `proofreaderWorkflow`
  - `tiptapEditWorkflow`
  - `streamText`
...
- 为 `diffUtility` 新增 `smartInlineV2` 差异模式。该模式改进了 `smartInline`，可正确显示块元素（例如表格）中的更改。
...
- 为 `executeTool`、`streamTool` 和 `tiptapEditWorkflow` 新增 `tiptapEditHooks` 选项，允许在应用 Tiptap 编辑操作前拦截、修改或拒绝操作。`beforeOperation` 钩子接受操作上下文，返回 `BeforeOperationResult`，用于控制操作的应用及其修改。
...
- 修正 `editThreads` 工具及评论工作流中，线程有时创建于错误位置的 Bug。

## 3.0.0-alpha.59

### 小幅更新

- 为 `editThreads` 工具新增操作追踪的流式支持，通过计数在扩展存储追踪，防止流式操作时重复执行。工具调用 ID 变更时，存储重置。
- 评论工作流新增流式支持。`editThreadsWorkflow` 增加 `isStreaming` 选项，设置为 true 时执行所有操作但不执行最后一个；为 false 或未定义时执行全部（含最后一个）。操作通过计数追踪防止重复，工作流 ID 变更时存储重置。
...
- 为 `diffUtility` 新增 `smartInline` 差异模式。该模式结合块级和内联比较，先识别变更块，再在块内执行字符级差异。适合文档部分内容修改，兼备效率和细节。

## 3.0.0-alpha.57

### 重大更新

- **破坏性变更**：重构 Tiptap 编辑实现，提升效率与准确性。移除 `HashAttributeExtension`，增强 schema 处理和哈希生成。
- **破坏性变更**：`tiptapRead` 方法返回类型中 `nodeRange` 重命名为 `contentRange`，增强清晰度。
- **破坏性变更**：工作流用户提示中，将 `nodes` 输入参数重命名为 `content`，影响 `tiptapEditWorkflow` 和 `editThreadsWorkflow`。请更新调用代码。

### 小幅更新

- 统一所有 Tiptap 方法中 `selectableNodeTypes` 的处理。
...
- 简化建议 API，合并普通建议和“比较文档”的更改为单一方法集。
...
- 为 `Suggestion` 接口新增 `reviewMode` 字段，指示建议处于“预览”还是“审核”模式。该字段由 `reviewOptions.mode` 自动设置。
- 新增 `rejectSuggestion` 和 `rejectAllSuggestions` 方法用于拒绝建议，并返回 AI 针对拒绝更改的反馈。
- 修改 `acceptSuggestion` 和 `acceptAllSuggestions` 方法，返回包含用户接受或拒绝更改的 `aiFeedback`，其中含删除和插入内容及状态，用于指导 AI 改进建议。
...
- 为 `diffUtility` 增加块级差异对比选项。`mode` 参数可设为 `block`，按顶级节点比较文档，适用于仅需了解段落或标题变动且不需要字符级细节的场景。

### 修复更新

- 修正 `diffUtility` 中使用 `changeMergeDistance` 时，不同顶级节点相邻改动被错误合并的问题，确保不同块改动分离。
- 修复 `editThreads` 工具 Bug：未读取文档即创建线程，线程不会被创建。
- 确保 `startComparingDocuments` 中被比文档与主文档使用同一 schema。
...
- 修正 `setHtmlSuggestions` 和 `setMarkdownSuggestions` 替换文本时错误修改 HTML 标签的问题。例如替换 "a" 为 "an" 不会将标签 `<a>` 错误替换为 `<an>`，且如果删除文本不含 `<` 或 `>`，替换操作仅影响文本，不修改标签。
...
- 使用 `diffUtilityOptions` 替代 `changeMergeDistance`，以提供更丰富的文档比较工具自定义选项。

### 小幅更新

- 在 `setSuggestions` 的 `compareDocuments` 格式中新增 `diffUtilityOptions` 选项，允许配置文档比较工具。
- 在 `Suggestion` 类型的 `displayOptions` 中新增 `diffAttributes` 选项，允许自定义差异装饰的属性。
...
- 为 `setHtmlSuggestions` 新增 `reviewOptions` 参数，支持用户定制建议显示选项（如 `diffPosition`、`showAsDiff`、自定义 `renderDecorations`）和自定义元数据，模式与其他编辑方法一致。
- 修正 `setHtmlSuggestions` 无 `range` 参数时偏移计算错误，确保建议高亮准确，修复位置偏移 -1 的 Bug。
...
- 优化 `createThread` 失败时 AI 接收到的错误信息。
...
- `insertContent` 工具新增 `activeNodeRange` 位置参数，允许 AI 在活动节点范围内插入内容。禁用 `applyDiff` 工具时可提升编辑准确率。
- 需升级以下提供者库至指定版本或更高：
...
- 修复 `diffUtility` 错误地将两个完全相同的空文档判断为不同的问题。
...
- 修复 `executeTool` 参数中 `input` 类型，使之兼容 Vercel AI SDK。
