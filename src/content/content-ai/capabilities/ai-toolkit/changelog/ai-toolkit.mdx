...
- 确保调用 `tiptapRead` 工具时，所有可选择节点均正确设置。修复 `tiptapRead` 工具返回内容中并非所有可选择节点都被设置的问题。
- 修复在使用 AiToolkit 扩展结合 Collaboration 扩展和外部创建的 Yjs 提供者时，页面每次重新加载文档中空段落累计的 Bug。
- 修复在预览模式下通过 tiptapEdit 更改文档节点结构后，tiptapRead 返回损坏内容的问题。
- 修复当 AI 生成的 HTML 包含如 `<tbody>`、`<thead>` 或 `<tfoot>` 等标准表格包装元素时，插入表格失败并报“无效的 HTML 内容”错误。

## 0.1.0

### 小幅更新

- 启动新的版本管理方案，使所有 AI 工具包和服务器端 AI 工具包包版本保持一致。
