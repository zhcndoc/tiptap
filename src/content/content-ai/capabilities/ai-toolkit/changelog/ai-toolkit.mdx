...
- 启用 `simplifyChanges` 时，单字符词内编辑现扩展至整词边界。
- 现在支持单字符插入、删除和替换，使差异更易阅读。
...
- 使用 Tiptap 位置工具进行位置映射。新版本需要 Tiptap 3.12.0 及以上。
- 全新流处理实现，允许 AI 流式期间其他用户编辑文档。影响操作：
  - tiptapEdit 工具
  - proofreaderWorkflow
  - tiptapEditWorkflow
  - streamText
...
- 为 `diffUtility` 新增 `smartInlineV2` 差异模式。该模式改进了 `smartInline`，正确展示区块元素（如表格）内的更改。
...
- 为 `executeTool`、`streamTool` 和 `tiptapEditWorkflow` 新增 `tiptapEditHooks` 选项，允许在应用 Tiptap 编辑操作前拦截、修改或拒绝操作。`beforeOperation` 钩子接收操作上下文，返回 `BeforeOperationResult`，控制操作应用与否及修改。

## 3.0.0-alpha.62

### 重大更新

- 更新 Tiptap 编辑工具的 `selectableNodeTypes` 参数，默认 AI 可选择并替换文档任意块节点。
...
- 为 `editThreads` 工具新增带操作跟踪的流式支持，操作通过计数在扩展存储追踪，避免流式中重复执行。工具调用 ID 变更时存储重置。
- 评论工作流新增流式支持。`editThreadsWorkflow` 新增 `isStreaming` 选项，为 true 时执行所有操作但不包含最后一个，为 false 或未定义时执行所有操作（含最后一个）。操作同样被计数追踪以防重复，工作流 ID 变更时存储重置。
...
- `executeTool` 和 `streamTool` 返回类型中 `output` 属性现返回 JSON 对象，便于日志记录与解析工具输出。
...
- **破坏性变更**：`tiptapRead` 方法返回类型中 `nodeRange` 重命名为 `contentRange`，提升清晰度。
...
- 将 `addHashesToDocument` 工具移动至 `tiptap-read` 目录优化结构。
...
- 移除“比较文档”功能中的 `acceptChange`、`rejectChange` 和 `rejectAllChanges`，改用 `acceptSuggestion`、`rejectSuggestion`、`acceptAllSuggestions` 和 `rejectAllSuggestions`，支持两者场景。
- 重命名 `applySuggestion` 为 `acceptSuggestion`，`applyAllSuggestions` 为 `acceptAllSuggestions`，保持一致且清晰。

### 小幅更新

- 为 `Suggestion` 接口新增 `reviewMode` 字段，指示建议处于“预览”或“审核”模式。该字段由 `reviewOptions.mode` 自动设置。
- 新增 `rejectSuggestion` 和 `rejectAllSuggestions`，用于拒绝建议，返回 AI 针对拒绝更改的反馈。
...
- 为 `diffUtility` 增加区块级差异对比选项。`mode` 参数可设为 `block`，按顶级节点比较文档，适合只需知晓段落或标题变化，且不追求字符细节的场景。

### 修复更新

- 修正 `diffUtility` 中，使用 `changeMergeDistance` 时不同顶级节点相邻改动被错误合并的问题，确保不同块改动分离。
- 修复 `editThreads` 工具BUG：文档未读取即创建线程，线程不会创建。
...
- 新增 `setMarkdownSuggestions` 方法，通过比较 Markdown 内容与当前文档显示建议，须安装配置 Tiptap Markdown 扩展。
- 新增 `getMarkdownRange` 方法，提取文档指定范围的 Markdown 内容，须安装配置 Tiptap Markdown 扩展。

### 修复更新

- 修正 `setHtmlSuggestions` 和 `setMarkdownSuggestions` 中替换文本时错误修改 HTML 标签的问题。如替换 "a" 为 "an" 不再误改 `<a>` 为 `<an>`，若删除文本无 `<` 或 `>`，替换仅影响文本，标签保持不变。
...
- 用 `diffUtilityConfig` 替代 `changeMergeDistance`，支持更丰富的文档比较定制。
...
- 修正文档更新后 AI 读取错误文档内容的 Bug。

## 3.0.0-alpha.39

### 修复更新

- 修复新线程和评论未创建于正确文档位置的 Bug。
...
- 使 `insertContent` 工具内容格式独立，将原 `"html"` 属性重命名为 `"content"`。
- 需升级以下提供者库至指定或更高版本：
...
- 修复 `insertContent` 工具中 `activeNodeRange` 位置无效 Bug。
- 修复 `insertContent` 导致 `activeNodeRange` 总重置为 0 的问题。
- 修正 `setHtmlSuggestions` 边界错误及格式丢失。边界用 `content.size` 替代 `nodeSize - 1`，应用建议时保留 HTML 格式（加粗、斜体、链接等）。

## 3.0.0-alpha.35

### 修复更新

- 修复 `streamText` 和 `streamHtml` 方法不必要将 ReadableStream 转 AsyncIterable 导致性能下降的问题。

## 3.0.0-alpha.34

### 小幅更新

- 为 `diffUtility`、`startComparingDocuments`、`setSuggestions`（`compareDocuments` 格式）、`setHtmlSuggestions` 及所有编辑方法的 `reviewOptions` 添加 `changeMergeDistance` 参数，允许合并相近改动，设定最大改动间文档位置距离做合并。
- 为 `setHtmlSuggestions` 新增 `reviewOptions` 参数，用户可定制建议显示选项（如 `diffPosition`、`showAsDiff`、自定义 `renderDecorations`）及添加自定义元数据，模式与其他编辑方法一致。
...
- 新增 `getSelectedSuggestion` 方法，获取当前选中的建议。
...
- 修复 `createThread` 操作中线程偶现创建于错误位置的 Bug。
...
- `insertContent` 工具新增 `activeNodeRange` 位置，允许 AI 在活动节点范围插入内容。禁用 `applyDiff` 工具时提升编辑准确率。
- 需升级以下提供者库至指定或更高版本：
...
- 修正流式显示详细差异导致差异持续更新，现仅流结束时显示详细差异。
- 修正流式内容开头插入多余段落。
...
- 使 `streamHtml` 和 `streamText` 方法返回流结束时 resolve 的 Promise。
...
- 在审核选项中将 `detailedDiff` 重命名为 `diffMode`，控制原文与修改内容差异显示方式。

## 3.0.0-alpha.15

### 重大更新

- 审核选项新增 `detailedDiff`，控制是否以详细差异形式显示更改。将原 `showDiff` 选项改名为 `detailedDiff`，语义更清晰。
- 工具执行和流处理选项中新增 `detailedDiff`，默认 `true`，更改默认以详细差异显示。
...
- 新增 `getActiveNodeRange` 方法，获取活动节点范围，替代旧 `currentChunk`，提升编辑准确性。
- 新增 `setActiveNodeRange` 方法，设置活动节点范围。
- 移除 `readFirstChunk`、`readNextChunk`、`readPreviousChunk` 工具。
- 新增 `readNodeRange` 工具，允许 AI 读取任意顶级节点范围，灵活性更高。

### 小幅更新

- 新增 `streamTool` 方法，向编辑器流式输入工具调用结果。
...
- 新增 `streamText` 和 `streamHtml` 方法，向编辑器流式输入文本和 HTML。
...
- 新增 `replaceDocumentWhereChanged` 方法，支持高效替换整个文档，仅替换发生改变部分，避免全内容标记为替换。
- 重写 `applyHtmlDiff` 和 `rejectAllChanges`，内部基于 `replaceDocumentWhereChanged`。
...
- 新增 `applySuggestionToOtherDoc`，用于实时比较文档时将建议应用至另一文档。
- 首次调用 `startComparingDocuments` 时忽略防抖超时，提升响应速度。

## 3.0.0-alpha.4

### 重大更新

- 移除 `executeTool` 返回值中的 `needsReview`，改用 `docChanged`。
- 当工具修改文档时，`docChanged` 设为 `true`。

## 3.0.0-alpha.3

### 小幅更新

- `executeTool` 返回类型新增 `needsReview`，标示调用是否需要用户审核。
