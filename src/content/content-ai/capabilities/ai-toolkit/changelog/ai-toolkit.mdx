...
- 0d86e00: `acceptAllSuggestions` 现在返回最后一个被接受建议的范围；如果没有接受任何建议，则返回 `null`。

### Patch Changes

- c7785ae: 移除 `acceptSuggestion` 中对 `transaction` 属性的支持，恢复之前的建议接受行为。
...
- **破坏性变更**：将 `ReviewOptions` 中的 `diffMode` 属性替换为 `useDiffUtility`（`boolean`）。将 `diffMode: 'detailed'` 改为 `useDiffUtility: true`，将 `diffMode: 'full'` 改为 `useDiffUtility: false`。
...
- 将 `changeMergeDistance` 选项替换为 `diffUtilityConfig`，为文档比较工具提供更多自定义选项。

### 小幅更新

- 在 `setSuggestions` 方法的 `compareDocuments` 格式中添加 `diffUtilityConfig` 选项。这允许配置文档比较工具。
- 在 `Suggestion` 类型的 `displayOptions` 中添加 `diffAttributes` 选项，允许自定义差异装饰的属性。
