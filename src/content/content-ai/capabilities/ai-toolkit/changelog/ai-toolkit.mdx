...
- **Breaking**: 添加新的 `smart` 差异模式，并将其设为默认。该差异模式可智能适配块级和行内内容。改动不会跨越块边界，从而为结构性文档变更生成更干净的差异。
- **Breaking**: 将 `detailed` 差异模式重命名为 `inline`。
- **Breaking**: 将 `ReviewOptions` 中的 `diffMode` 属性替换为 `useDiffUtility`（`boolean`）。将 `diffMode: 'detailed'` 改为 `useDiffUtility: true`，将 `diffMode: 'full'` 改为 `useDiffUtility: false`。
- **Breaking**: 将默认的 `changeMergeDistance` 设置为 `10`。
- **Breaking**: 将 `displayOptions.showAsDiff` 重命名为 `displayOptions.showReplacement`。
- **Breaking**: 移除 `displayOptions.diffPosition`。
- **Breaking**: 将 `displayOptions.diffAttributes` 重命名为 `displayOptions.replacementAttributes`。
...
- 为 `diffUtilityOptions`（`ExternalDiffUtilityOptions`）添加 `groupInlineChanges`，用于控制何时将行内编辑分组为一个建议。
- 添加 `displayOptions.showSubChanges`、`displayOptions.subChangeAttributes` 和 `displayOptions.replacementSubChangeAttributes`，用于自定义分组后的行内更改。
- 为分组后的行内更改在 `Suggestion` 中添加 `isInlineGroup`。
- 在 `proofreaderWorkflow` 中，行内更改默认不再分组。需要时可通过 `reviewOptions.diffUtilityOptions.groupInlineChanges` 覆盖。
...
- 为 `diffUtility`、`startComparingDocuments`、`setSuggestions`（compareDocuments 格式）、`setHtmlSuggestions` 和所有编辑方法的 `reviewOptions` 新增 `changeMergeDistance` 选项。通过指定需要合并的更改之间的最大距离（以文档位置计），可在比较文档时合并相邻更改为单个更改。
- 为 `setHtmlSuggestions` 方法新增 `reviewOptions` 参数。现在用户可以像其他编辑方法一样，自定义建议显示选项（如 `diffPosition`、`showReplacement`、自定义 `renderDecorations`）并添加自定义元数据。
