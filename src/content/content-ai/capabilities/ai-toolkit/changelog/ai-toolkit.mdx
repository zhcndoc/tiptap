...
- 建议非阻塞：如果文档已有建议，AI 可以继续编辑并在现有建议基础上叠加更多建议。用户可在最后审阅所有建议。
- **破坏性变更**：`tiptapRead` 现在返回 `TiptapReadResult`。
- **破坏性变更**：`tiptapRead` 接受 `TiptapReadResult` 类型的参数。
- **破坏性变更**：向文档添加全局属性 `_hash`，辅助多层变更时的节点选择。
- **破坏性变更**：移除可配置的 `hashTypes` 和 `selectableNodeTypes` 选项。现在所有非内联节点均可选择。
- **破坏性变更**：校对操作类型中，将 `hash` 重命名为 `target`，以与编辑操作保持一致。

## 3.0.0-alpha.87

### 小幅更新

- 新增 `invertSuggestions` 方法。该方法执行模拟运行，将所有当前建议应用至文档副本，返回结果文档及可撤销这些更改的反向建议。反向建议保留原建议的元数据。预览模式的建议在反向结果中转换为审核模式，反之亦然。

## 3.0.0-alpha.86

### 小幅更新

- 为 `diffUtility` 和 `ExternalDiffUtilityOptions` 添加 `expandBlockChanges` 选项。当匹配节点类型包含于指定列表，且其所有内联内容均已更改时，将个别内联改动替换为覆盖整个节点的单一块级改动。默认值为 `['listItem']`。仅在模式为 `'smart'` 时生效。

### 修复更新

- 修复当差异改动跨越不能作为直接 `doc` 内容的节点（如未包裹的 `listItem` 节点）时，替换装饰渲染导致崩溃问题。渲染时先将内容包裹在必要的父节点内，再应用子改动标记。

## 3.0.0-alpha.85

### 小幅更新

- 为 `templateWorkflow` 和 `fillTemplate` 添加 `preserveSlotAttr` 选项。启用后，填充后节点会保留 `_templateSlot` 属性，便于识别 AI 填充内容及重新填充模板字段。
- 为 `fillTemplate` 增加相邻槽合并功能。多个连续具有相同 `_templateSlot` 属性值的节点被视作单一槽，一同替换为一个 HTML 输出。
...
- 新增 `TemplateField` Tiptap 扩展，为所有节点类型注册 `_templateSlot`、`_templateIf` 和 `_templateAttributes` 全局属性。该扩展提供命令（`setTemplateSlot`、`unsetTemplateSlot`、`setTemplateIf`、`unsetTemplateIf`、`setTemplateAttributes`、`unsetTemplateAttributes`）用于设置或取消当前选中节点的模板属性。
...
- **破坏性变更**：简化 `InsertPosition` 类型，由原联合类型 8 个选项改为 `Range | number`。位置参数现在可为数字偏移或 `Range` 对象，默认值为整个文档范围。
- **破坏性变更**：移除 `setHtmlSuggestions` 方法，建议改用校对工作流（Proofreader Workflow）。
- **破坏性变更**：移除 `smartInline` 和 `smartInlineV2` 差异模式，建议改用 `smart`。
- **破坏性变更**：简化 `setSuggestions` 和 `addSuggestions` 方法，改为直接接受 `Suggestion[]`，不再支持 `SuggestionsFormat[]`。`compareDocuments` 格式也不再支持。建议改用新方法 `setSuggestionsFromDiff`。
- **破坏性变更**：简化 `ReplacementOption` 类型，由判别联合类型（`SliceReplacementOption` 或 `TextReplacementOption`）改为包含 `content` 属性（类型为 `string | Slice`）的单一接口，移除 `type`、`addSlice` 和 `addText` 属性。

### 小幅更新

- 新增 `setSuggestionsFromDiff` 方法，支持比较两个文档并根据差异设置建议，用以替代旧版 `setSuggestions` 中 `compareDocuments` 格式。
...
- 修正 `simplifyChanges` 误将改动范围扩展至单词边界之外的错误。现相邻的非字母字符（如空格、标点符号）不会被包含在改动范围内。单词边界计算同时检测原文和修改后文档内容。
...
- **破坏性变更**：新增默认差异模式 `smart`，智能适配块级与内联内容。改动永不跨块边界，适用于结构化文档编辑，生成清晰差异结果。
...
- **破坏性变更**：所有方法与选项中的 `diffUtilityConfig` 属性重命名为 `diffUtilityOptions`。
...
- 在 `diffUtility` 和 `diffUtilityOptions` 中新增 `groupInlineChanges`，用于控制内联改动何时合并为单一建议。
- 在 `displayOptions` 中新增 `showSubChanges`、`subChangeAttributes` 和 `replacementSubChangeAttributes`，支持自定义分组内联改动显示。
...
- 修复 ES2025 迭代器辅助函数（如 `Map.entries().filter()`、`.map()` 等）在 Safari < 18.4、Firefox < 131 等不支持特性的浏览器上导致 `TypeError` 的兼容性问题。
...
- 为 `executeTool`、`streamTool` 和 `tiptapEditWorkflow` 增加 `tiptapEditHooks` 选项，允许在应用 Tiptap 编辑操作前拦截、修改或拒绝操作。`beforeOperation` 钩子接收操作上下文，返回 `BeforeOperationResult`，控制操作是否应用及可能的修改。
...
- 为 `editThreads` 工具新增操作追踪流式支持，利用计数器在扩展存储中追踪和防止重复执行。工具调用 ID 变更时重置存储。
- 评论工作流新增流式支持。`editThreadsWorkflow` 添加 `isStreaming` 选项，设为 true 时执行所有操作但跳过最后一个；为 false 或未设置时执行全部操作（含最后一个）。操作计数机制防止重复执行，工作流 ID 变更时重置存储。
...
- `executeTool` 和 `streamTool` 返回类型中，`output` 属性现返回 JSON 对象，便于日志记录和工具输出解析。
...
- 将 `addHashesToDocument` 工具移至 `tiptap-read` 目录，优化结构。
...
- 新增 `getThreads` 方法，获取文档所有线程及其评论和位置。
...
- 为 `Suggestion` 接口新增 `reviewMode` 字段，用于指示建议处于“预览”还是“审核”模式。此字段由 `reviewOptions.mode` 自动设置。
- 新增 `rejectSuggestion` 和 `rejectAllSuggestions` 方法，用以拒绝建议并返回 AI 关于拒绝更改的反馈。
