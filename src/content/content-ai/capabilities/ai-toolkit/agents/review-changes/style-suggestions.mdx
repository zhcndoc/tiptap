...
当 AI 对文档进行更改时，您可以在编辑器中将这些更改显示为[建议](/content-ai/capabilities/ai-toolkit/api-reference/suggestions)。在本指南中，您将学习如何通过修改样式来自定义建议的外观，并在其中显示自定义元素（例如菜单）。
...
在[预览模式](/content-ai/capabilities/ai-toolkit/agents/review-changes#two-types-of-suggestions)中使用这些样式，以便在更改插入文档之前预览它们。
...
在[审阅模式](/content-ai/capabilities/ai-toolkit/agents/review-changes#two-types-of-suggestions)中使用这些样式，用于在文档中插入后审阅更改。
...
- 在[API 参考](/content-ai/capabilities/ai-toolkit/api-reference/suggestions)中了解更多有关建议的信息。请查看 `displayOptions` 参数以了解所有自定义选项。
