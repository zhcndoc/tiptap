...
title: 建议
meta:
  title: 使用建议审阅 AI 变更 | Tiptap Content AI
...
你可以将本指南与 [Streaming](/content-ai/capabilities/ai-toolkit/agents/streaming) 结合使用，以便在建议被审核时，流式传输操作并保持助手的响应性。

### 自定义建议的显示方式

在审核更改时，AI Toolkit 会将更改的预览显示为 [建议](/content-ai/capabilities/ai-toolkit/api-reference/suggestions)。

你可以通过设置 `reviewOptions.displayOptions` 参数来自定义这些建议的显示方式。你甚至可以在其中 [渲染 React 组件](/content-ai/capabilities/ai-toolkit/agents/review-changes/style-suggestions)。
