...
如果您的后端不是 TypeScript，可以使用 CLI 生成包含工具定义的 JSON 文件。详见[非 TypeScript 后端](/content-ai/capabilities/ai-toolkit/advanced-guides/non-typescript-backends) 指南。
...
返回包含启用工具定义的数组，格式为通用格式。每个工具定义都是包含 `name`、`description` 和 `inputSchema` 属性的对象。要查看完整工具列表，请参见 [可用工具](/content-ai/capabilities/ai-toolkit/agents/tools) 页面。
