...
export async function POST(req: Request) {
  ...
  const agent = new ToolLoopAgent({
    ...
    tools: toolDefinitions({
      tools: {
        // Enable tools to read and edit comments
        getThreads: true,
        editThreads: true,
        // (Optional) Prevent AI from editing document content
        tiptapEdit: false,
      },
    }),
  })
...
export function CommentsAiChatbot({ editor }) {
  ...
  // Fix issue: https://github.com/vercel/ai/issues/8148
  ...
  const { messages, sendMessage, addtoolOutput } = useChat({
    ...
    async onToolCall({ toolCall }) {
      ...
      // Execute the tool using AI toolkit
