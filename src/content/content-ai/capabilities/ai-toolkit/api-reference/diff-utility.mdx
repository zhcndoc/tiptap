...
- `displayOptions?` (`DisplayOptions`)：控制差异如何以建议的形式呈现。[查看可用选项](/content-ai/capabilities/ai-toolkit/api-reference/suggestions#the-suggestion-object)。
- `debounceTimeout?` (`number`)：重新计算 diff 之前等待的毫秒数。较低的值会让应用看起来更响应，但在大型文档中可能会导致性能问题。默认值：`500`
- `diffUtilityOptions?` ([`ExternalDiffUtilityOptions`](/content-ai/capabilities/ai-toolkit/api-reference/diff-utility#parameters-diffutilityoptions))：使用 diff 实用工具比较文档时的选项。[查看可用选项](/content-ai/capabilities/ai-toolkit/api-reference/diff-utility#parameters-diffutilityoptions)。
...
- [`acceptSuggestion`](/content-ai/capabilities/ai-toolkit/api-reference/suggestions#acceptsuggestion)：接受特定变更
- [`rejectSuggestion`](/content-ai/capabilities/ai-toolkit/api-reference/suggestions#rejectsuggestion)：拒绝特定变更
- [`acceptAllSuggestions`](/content-ai/capabilities/ai-toolkit/api-reference/suggestions#acceptallsuggestions)：接受所有变更
- [`rejectAllSuggestions`](/content-ai/capabilities/ai-toolkit/api-reference/suggestions#rejectallsuggestions)：拒绝所有变更
