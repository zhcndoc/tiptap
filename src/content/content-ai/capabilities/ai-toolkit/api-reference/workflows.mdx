...
- `options` (`ProofreaderWorkflowOptions`): 配置选项
  ...
  - `reviewOptions?` (`ReviewOptions`): 控制预览/审核行为
    ...
    - `diffUtilityConfig?` (`DiffUtilityConfig`): 用于比较文档的差异工具的选项
      - `simplifyChanges?` (`boolean`): 是否简化更改。如果为 `true`，更改将被简化，例如，将同一单词中的两个更改合并为一个更改。仅在模式为 'detailed' 或 'smartInline' 时适用。默认值：`true`
      ...
      - `mode?` (`'detailed' | 'block' | 'smartInline'`): 用于比较的差异模式。`'detailed'` 执行字符级别的差异，识别文本中的精确更改（默认）。`'block'` 执行块级差异，将每个顶层节点视为一个单元。块模式在只需知道哪些段落、标题或其他块级元素发生变化时非常有用，而不需要字符级的细节。在块模式下，`simplifyChanges` 和 `changeMergeDistance` 选项将被忽略。`'smartInline'` 结合了两者：首先识别更改的块，然后在这些块内执行内联差异。适用于只改变部分内容的文档，同时仍提供字符级精度。默认值：`'detailed'`
