...
- `_templateSlot`（`string | null`）：标记节点为内容占位符。填充模板时，该节点将被 AI 生成的 HTML 替换。
- `_templateIf`（`string | null`）：标记节点为条件包含。当值为 `false` 时，该节点将从输出中移除。
- `_templateAttributes`（`TemplateAttributeEntry[] | null`）：映射模板键到具体节点属性。
