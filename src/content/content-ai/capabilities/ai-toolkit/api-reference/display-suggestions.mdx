...
- `diffUtilityConfig?` (`DiffUtilityConfig`): 差异工具的对比参数
  ...
  - `mode?` (`'detailed' | 'block' | 'smartInline'`): 对比模式。`'detailed'` 执行字符级差异（默认），`'block'` 按顶级节点块级别比较。块级模式忽略 `simplifyChanges` 和 `changeMergeDistance`。`'smartInline'` 结合两者：先识别变更块，再在这些块中进行字符级差异。默认：`'detailed'`
...
- `options` (`SetHtmlSuggestionsOptions`): 配置项
  ...
  - `diffUtilityConfig?` (`DiffUtilityConfig`): 差异对比工具配置
    ...
    - `mode?` (`'detailed' | 'block' | 'smartInline'`): 对比模式，默认为 `'detailed'`（字符级差异）
