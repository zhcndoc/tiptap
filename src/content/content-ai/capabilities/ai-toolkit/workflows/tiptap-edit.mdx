...
要仅编辑文档的子区域，请设置 `range` 选项。该参数接收一个带有待编辑区域的 [Range](/content-ai/capabilities/ai-toolkit/advanced-guides/concepts#range)。
...
对于大型文档，你可以使用 [`tiptapReadChunks`](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapreadchunks) 方法，将文档拆分为多个块并进行并行处理。每个块都包含其区域范围的 [Range](/content-ai/capabilities/ai-toolkit/advanced-guides/concepts#range)。

## 显示复审界面

要让用户复审修改内容，可以配置 [复审选项](/content-ai/capabilities/ai-toolkit/api-reference/review-options)。修改将以建议的形式显示，用户可以接受或拒绝。
...
如果需要在应用工作流后获取每个操作的执行状态，请读取 `result.operationResults`。

## 将更改作为跟踪更改进行复审

如果安装了 [跟踪更改](/tracked-changes/getting-started/overview) 扩展，`tiptapEditWorkflow` 可以将 AI 编辑应用为跟踪更改，而不是预览建议。
...
请参阅 [建议](/content-ai/capabilities/ai-toolkit/advanced-guides/suggestions) 指南以了解更多关于建议的信息。

## 相关指南

- [`tiptapRead` 方法的 API 参考](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapread)
- [`tiptapReadChunks` 方法的 API 参考](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapreadchunks)
- [`tiptapEditWorkflow` 方法的 API 参考](/content-ai/capabilities/ai-toolkit/api-reference/workflows#tiptapeditworkflow)
- [建议](/content-ai/capabilities/ai-toolkit/advanced-guides/suggestions)：了解文档复审和在编辑器中显示建议的相关知识。
- [AI 光标](/content-ai/capabilities/ai-toolkit/advanced-guides/ai-caret)：显示 AI 插入内容位置的光标。
