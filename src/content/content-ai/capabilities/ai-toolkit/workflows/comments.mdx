...
要只对文档的某一子部分添加评论，可以在 `tiptapRead` 上设置 `range` 选项。该参数接收一个带有指定区域的 [Range](/content-ai/capabilities/ai-toolkit/advanced-guides/concepts#range)。
...
对于大型文档，可以使用 [`tiptapReadChunks`](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapreadchunks) 方法将文档拆分为多个块并行处理。每个块都包含一个带有该块区域的 [Range](/content-ai/capabilities/ai-toolkit/advanced-guides/concepts#range)。
