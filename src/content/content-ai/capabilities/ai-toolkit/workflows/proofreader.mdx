...
要校对文档的子区域，请设置 `range` 选项。此参数接受一个描述文档将被校对区域的 [Range](/content-ai/capabilities/ai-toolkit/advanced-guides/concepts#range)。
...
对于大型文档，你可以使用 [`tiptapReadChunks`](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapreadchunks) 方法，将文档拆分成多个块并平行处理。每个块都包含一个 [Range](/content-ai/capabilities/ai-toolkit/advanced-guides/concepts#range) 以指明该块的区域。
...
请参阅 [审阅更改](/content-ai/capabilities/ai-toolkit/agents/review-changes) 指南，了解有关审阅界面的更多信息。

请查看 [AI Toolkit 演示](https://github.com/ueberdosis/ai-toolkit-demos)，了解如何将 AI Toolkit 工作流程与审阅界面结合使用的示例。

## 相关指南

- [API 参考](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapread) of the `tiptapRead` method
- [API 参考](/content-ai/capabilities/ai-toolkit/api-reference/read-the-document#tiptapreadchunks) of the `tiptapReadChunks` method
- [API 参考](/content-ai/capabilities/ai-toolkit/api-reference/workflows#proofreaderworkflow) of the `proofreaderWorkflow` method
- [建议](/content-ai/capabilities/ai-toolkit/agents/review-changes)：了解如何审阅文档并在编辑器中显示建议。
