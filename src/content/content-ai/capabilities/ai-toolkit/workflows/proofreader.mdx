...
配置 [review options](/content-ai/capabilities/ai-toolkit/api-reference/review-options)，显示校对建议，允许用户接受或拒绝。
...
如果需要在应用工作流程后获取每个操作的执行状态，请读取 `result.operationResults`。

## 以修订记录形式审阅更改

如果安装了 [修订记录](/tracked-changes/getting-started/overview) 扩展，`proofreaderWorkflow` 可以将更正应用为修订记录，而不是预览建议。
...
- [显示建议](/content-ai/capabilities/ai-toolkit/advanced-guides/suggestions)：了解如何审阅文档并在编辑器中显示建议。
