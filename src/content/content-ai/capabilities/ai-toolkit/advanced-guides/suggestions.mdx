...
建议是在调用 AI 工具包中[编辑文档](/content-ai/capabilities/ai-toolkit/api-reference/edit-the-document)的方法时生成的，并设置了[审阅选项](/content-ai/capabilities/ai-toolkit/api-reference/review-options)。
...
您可以通过 `reviewOptions` 对象的 `diffUtilityOptions` 属性配置比较方式。
...
- `smart`（默认）：平衡模式，便于生成易读的审阅建议。
- `inline`：显示文本中精确的字符级更改。
