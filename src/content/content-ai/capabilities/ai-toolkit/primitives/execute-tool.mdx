...
返回 `activeNodeRange` 变量的值。

活动节点范围是 AI 刚刚读取的节点范围，AI 能够在下一次工具调用中编辑该范围。它使 AI 只能编辑文档中特定的节点范围，避免在文档过大时超出 AI 模型的上下文窗口。

### 返回值

- `NodeRange`: 活动节点范围，具有 `from` 和 `to` 属性，表示起始和结束的节点索引。
...
设置 `activeNodeRange` 变量。该变量用于执行带有 `executeTool` 或 `streamTool` 方法的工具调用时，确定 AI 在下一次工具调用中可以编辑的节点范围。

活动节点范围是 AI 刚刚读取的节点范围，AI 能够在下一次工具调用中编辑该范围。它使 AI 只能编辑文档中特定的节点范围，避免在文档过大时超出 AI 模型的上下文窗口。

### 参数

- `nodeRange` (`NodeRange`): 要设置的节点范围，具有 `from` 和 `to` 属性，表示起始和结束的节点索引。
