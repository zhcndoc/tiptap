...
meta:
  ...
  description: 通过生成式 AI 扩展实现自定义 LLM，并在编辑器中覆盖解析器函数。请查阅文档了解更多信息！
  category: 内容 AI
---

import { Callout } from '@/components/ui/Callout'

如果你想使用自己的后端来提供访问自定义 LLM 的能力，可以在扩展配置中覆盖以下定义的解析器函数。

请确保你返回的是正确类型的响应，并且正确处理错误。

<Callout title="温馨提示！" variant="hint">
  强烈建议你不要在前端直接调用 OpenAI 接口，这可能导致 API 令牌泄露！你应当通过后端代理来保护你的 API 令牌安全。
</Callout>
