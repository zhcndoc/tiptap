...
你从 AI 设置页面获取了一个 App ID 和 secret，并使用该 secret 签名了一个 JWT。App ID 和 token 被传递给了 AI 扩展。REST 请求在 `Authorization` 中携带 JWT，并在 `X-App-Id` 中携带 App ID。请参见 [内容 AI 安装](/content-ai/capabilities/generation/install)。
