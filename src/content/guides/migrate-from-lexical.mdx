...
从 [Lexical](https://lexical.dev/) 迁移到 Tiptap 很简单。本指南将帮助你从 Lexical 的基于节点的架构过渡到 Tiptap 的扩展系统。

## 内容迁移

### HTML 内容兼容性

Lexical 使用自己的 JSON 结构，需要转换为 Tiptap 可用格式。你可以将 Lexical 内容序列化为 HTML：
...
1. 安装 Tiptap 包
2. 将 Lexical JSON 内容转换为 HTML
3. 用 Tiptap 设置替换 Lexical 编辑器初始化
4. 将 Lexical 插件映射到 Tiptap 扩展
5. 将自定义节点迁移到 [节点视图](/editor/extensions/custom-extensions/node-views)
6. 将工具栏插件迁移到 Tiptap UI 组件
7. 测试内容兼容性
8. 将现有内容转换为 JSON 格式（推荐）
9. 对缺失功能实现自定义扩展
10. 更新事件处理和命令派发
11. 测试浮动工具栏功能
12. 若使用表格，验证表格功能
