...
从 [Quill](https://quilljs.com/) 迁移到 Tiptap 非常简单。本指南将帮助你顺利完成从 Quill 的 Delta 格式到 Tiptap 扩展系统的过渡。

## 内容迁移

### HTML 内容兼容性

Quill 使用 Delta 格式存储内容，需要先转换为 HTML 以供 Tiptap 使用：
...
虽然 HTML 已经完全兼容，我们建议转换为 Tiptap 的 JSON 格式以获得更好的性能和可读性。对于批量转换现有内容，可使用[HTML 工具](/editor/api/utilities/html)程序化地将 HTML 转为 JSON。
...
Tiptap 支持所有现代前端 UI 框架，例如 React 和 Vue。请参阅我们的[安装指南](/editor/getting-started/install)中针对具体框架的安装说明。
...
在我们的[扩展指南](/editor/extensions/overview)中探索所有可用的扩展，或者[创建自定义扩展](/editor/extensions/custom-extensions)以支持自定义功能和 HTML 元素。
...
对于 Quill 自定义模块或 blot，请创建对应的 Tiptap 自定义扩展。详见我们的[自定义扩展指南](/editor/extensions/custom-extensions)。
...
1. 安装 Tiptap 包
2. 将 Quill Delta 内容转换为 HTML
3. 用 Tiptap 设置替换 Quill 初始化
4. 映射 Quill 模块到 Tiptap 扩展
5. 迁移工具栏配置为自定义 UI 组件
6. 测试内容兼容性
7. （推荐）将现有内容转换为 JSON 格式
8. 针对缺失功能实现自定义扩展
9. 更新事件处理与 API 调用
10. 如果使用气泡主题，测试气泡菜单功能
11. 验证图片上传与处理
12. 测试格式和样式功能
