...
从 [Draft.js](https://draftjs.org/) 迁移到 Tiptap 非常简单。本指南将帮助你从 Draft.js 的不可变状态模型过渡到 Tiptap 的扩展系统。
...
1. 安装 Tiptap 包
2. 将 Draft.js ContentState 转换为 HTML
3. 使用 Tiptap 设置替换 Draft.js 编辑器
4. 映射 Draft.js 内联样式到 Tiptap 标记
5. 映射 Draft.js 块类型到 Tiptap 节点
6. 将实体迁移到[节点视图](/editor/extensions/custom-extensions/node-views)或扩展
7. 将工具栏组件迁移到 Tiptap UI
8. 测试内容兼容性
9. 转换现有内容为 JSON 格式（推荐）
10. 为任何缺失功能实现自定义扩展
11. 更新事件处理程序和按键命令
12. 测试行内工具栏功能
13. 验证链接和图片处理
