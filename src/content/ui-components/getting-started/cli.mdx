...
在空目录中运行时，它会搭建一个新的 Vite 或 Next.js 项目；在现有项目中运行时，它会对其进行配置。无论哪种方式，组件都会被复制到您的 `components/` 文件夹中——然后您可以在自己的页面中导入并渲染它们。它们不会自动接入页面，因此新搭建的应用仍会显示框架默认的起始页面，直到您渲染一个组件为止。有关完整流程，请参阅 [Next.js](/ui-components/install/next) 或 [Vite](/ui-components/install/vite) 指南。
