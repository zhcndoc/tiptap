...
CPA 最终答案传输协议要求：保留模型的原生决策。如果正确的下一步输出是用户原本提供的工具之一，则正常调用该原始工具。如果正确的下一步输出是普通的助手文本最终答案，而不是真实的工具调用，则准确调用一次 `cpa_final_answer`，并将完整的面向用户的自然语言答案放入其 `answer` 参数中。在 `answer` 中保留用户要求的语言、格式、Markdown、详细程度和简洁程度；答案完成后，追加一行最终内容，其中必须完全包含
...
|                        | 编辑器扩展                          | REST API                         |
| ---------------------- | ------------------------------------ | -------------------------------- |
| 运行位置               | 您的编辑器（客户端）                 | Tiptap 云（服务端）              |
| 脚注和尾注             | 通过 Pages 自动应用；数据在回调中提供 | 在响应中返回                    |
| 页眉和页脚             | 通过 Pages 扩展自动应用              | 作为单独的数据字段返回          |
| 未知内容               | 按您的编辑器架构重新写入             | 原始 JSON，不进行架构过滤       |
...
安装 [ConvertKit](/conversion/import/docx/convertkit)：它捆绑了导入器所需的每个扩展：标准节点和 mark，`TextStyleKit` 用于颜色/字体/大小/行高/背景，预配置为 `['paragraph', 'heading']` 的 `TextAlign`，带有 `multicolor: true` 的 `Highlight`，带有 DOCX 裁剪属性的 `Image`，用于支持 DOCX 的表格的 `ConvertTableKit`，以及 `PageBreak`：
...
如果您需要将捆绑扩展中的某一个替换为自己的实现，可以通过 `ConvertKit.configure({ slot: false })` 禁用任意单个 slot（例如，与 [Pages 扩展](/pages/getting-started/overview) 配合时，设置 `table: false`，并改为注册来自 `@tiptap-pro/extension-pages-tablekit` 的 `TableKit`）。完整的 slot 列表请参见 [ConvertKit 参考文档](/conversion/import/docx/convertkit)。
...
| 属性                | 描述                                                                                                                                                                                                                                                                      |
| ------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `token`             | 由您的服务器为 Convert 服务生成的 JWT。这是请求所需的唯一凭据。有关如何签发 JWT，请参阅[身份验证](/authentication)。                                                                                                                    |
| `endpoint`          | Convert API 的基础 URL。默认为 `https://api.tiptap.dev/v2/convert`，因此只有在需要指向其他部署时才需要设置此项。                                                                                                                                                  |
| `imageUploadConfig` | 导入期间图片上传的结构化配置。详情请参阅[图片上传配置](#image-upload-configuration)。                                                                                                                                             |
| `prosemirrorNodes`  | 重命名导入生成的节点类型，以适配使用不同名称的 schema。请参阅[自定义节点映射](/conversion/import/docx/custom-node-mapping)。                                                                                                                        |
| `prosemirrorMarks`  | 重命名导入生成的标记类型，并将 Word 字符样式映射到标记。请参阅[自定义标记映射](/conversion/import/docx/custom-mark-mapping)。                                                                                                                    |
| `cssStyles`         | 选择将文档的默认样式提取为 CSS，以便将其应用到编辑器。请参阅[CSS 注入](/conversion/import/docx/css-injection)。                                                                                                                       |
| `verbose`           | 一个用于控制导入返回多少诊断输出的 `number`。适用于调试，或进一步了解转换期间发生的情况。详情请参阅[详细输出](#verbose-output)。                                                                                 |
...
当导入包含页眉和页脚的 `.docx` 文件时，如果已安装 [Pages 扩展](/pages/getting-started/overview)，导入扩展会自动检测并应用它们。没有 Pages 时，页眉/页脚数据仍会返回给你的 `onImport` 回调（见下方 [手动处理](#manual-handling)），但没有地方可以渲染。请安装 Pages 以便它们在编辑器中可见。
...
Word 的页眉和页脚通常包含 `PAGE` 和 `NUMPAGES` 字段代码，它们会渲染为实时页码。默认情况下，当安装了 [Pages 扩展](/pages/getting-started/overview) 时，`importDocx` 会将这些字段转换为与你当前 Pages 注册表匹配的文本令牌（开箱即用为 `{page}` 和 `{total}`），从而在重新导出为 Word 时能够无缝地还原为实时字段，无需额外配置。
...
<Callout title="与 Tiptap Pages 自动应用" variant="hint">
  当 [Pages 扩展](/pages/getting-started/overview) 与
  [脚注](/pages/core-concepts/footnotes) 和/或 [尾注](/pages/core-concepts/endnotes)
  一起启用时，导入的注释会自动应用：标记会在正文中渲染，脚注内容会进入每页的脚注区域，
  尾注内容会进入文档末尾的尾注列表，并且编号与源文档保持一致。无需额外配置。
</Callout>

如果不使用 Pages，`footnoteReference` / `endnoteReference` 节点需要一个[自定义扩展](/editor/extensions/custom-extensions/extend-existing)才能在编辑器中渲染。`ImportContext` 上的数据已经提供了构建你自己的展示所需的一切。
