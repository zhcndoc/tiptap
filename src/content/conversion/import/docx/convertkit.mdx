...
带有 `*` 标记的扩展是 Tiptap 基础扩展的自定义 DOCX 感知覆盖版本。它们新增的内容见下一节。带有 `†` 标记的扩展虽然已捆绑，但**默认关闭**——可通过同名配置项启用（参见 [配置参考](#configuration-reference)）。

ConvertKit 还导出与 `OrderedListNumbering` 搭配使用、用于编辑器预览渲染的辅助函数和类型——`generateNumberingFormatCss`、`GenerateNumberingFormatCssOptions`、`NumberingFormatDefinition`、`NumberingLevelDefinition`、`NumberingBaseStyle` 和 `NumberingMarkerFont`。参见 [有序列表编号](/conversion/export/docx/ordered-list-numbering) 获取端到端方案。
...
- 编辑器挂载期间会应用一个小而作用域化的 CSS 重置。同一页面上的多个基于 ConvertKit 的编辑器会共享这份重置——它只会插入一次，并且只有在最后一个编辑器销毁时才会移除。
- 跨你的应用对 DOCX 导入内容进行一致的渲染。具有非零 DOCX 裁剪的图片会实际按裁剪后的效果渲染；当列宽缩小时，`heightRule: "atLeast"` 的行会在垂直方向增长；`verticalAlign: "center"` 的单元格会让其内容居中。
- 与 Tiptap Pro 其余扩展生态完全兼容。`@tiptap-pro/extension-import-docx` 和 `@tiptap-pro/extension-export-docx` 是天然的配套扩展；如果你需要分页，`@tiptap-pro/extension-pages-tablekit` 可以叠加其上。
- **紧凑的默认排版。** CSS 重置会清零段落外边距并应用紧凑行高。如果你将 ConvertKit 用于并非从 DOCX 导入的内容，请在其上叠加你自己的排版规则——这种重置是有意为之，而不是我们期望直接呈现精美视觉效果的默认值。
...
ConvertKit.configure({
  ...
  orderedListNumbering: boolean | Partial<OrderedListNumberingOptions>, // 默认：false — 通过 `true` 启用，或传入一个选项对象（`defaultFormat`、`formats`）
