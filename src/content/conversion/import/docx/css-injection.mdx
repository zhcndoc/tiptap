...
ImportDocx.configure({
  cssStyles: {
    enabled?: boolean,     // 默认值：false。启用提取。
    autoInject?: boolean,  // 默认值：false。将 <style> 标签追加到 document.head。
    selector?: string,     // 默认值：'.tiptap'。注入规则的作用域。
  },
})
```
...
editor.chain().importDocx({
  file,
  cssStyles: { enabled, autoInject, selector }, // 可使用任意子集
  onImport(context) { /* context.cssStyles 在此处可用 */ },
})
```

REST API 查询参数（如果直接调用 `/import/docx`）：
