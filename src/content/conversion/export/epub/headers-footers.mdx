...
| `differentFirstPage` | `boolean`  | 是否在第一页使用不同的页眉。设置为 `true` 时，第一页使用 `first` 值，而不是 `default`。                   |
| `default`           | `string`   | 每页的标准默认页眉，或启用 `evenAndOddHeaders` 时奇数页的页眉。支持纯文本字符串或序列化的 Tiptap JSONContent，以实现更丰富的格式。 |
| `first`             | `string`   | 第一页的页眉，仅在 `differentFirstPage` 为 `true` 时使用。支持纯文本字符串或序列化的 Tiptap JSONContent，以实现更丰富的格式。    |
| `even`              | `string`   | 偶数页的页眉，仅在启用 `evenAndOddHeaders` 选项时使用。支持纯文本字符串或序列化的 Tiptap JSONContent，以实现更丰富的格式。       |

使用纯文本时，将生成一个简单的无样式页眉。使用序列化的 Tiptap JSONContent（通过 `JSON.stringify()`）时，可以保留粗体、斜体和链接等丰富格式。
