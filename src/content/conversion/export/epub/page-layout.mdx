...
使用 `pageSize` 选项为导出的 EPUB 文件定义自定义页面尺寸。页面尺寸配置支持使用多种测量单位设置宽度和高度。
...
| 属性     | 类型     | 描述                                                                                              | 默认值        |
|----------|----------|---------------------------------------------------------------------------------------------------|---------------|
| `width`  | `string` | 页面宽度。必须是后跟有效单位（`cm`、`in`、`pt`、`pc`、`mm`、`px`）的正数。 | `"21cm"`      |
| `height` | `string` | 页面高度。必须是后跟有效单位（`cm`、`in`、`pt`、`pc`、`mm`、`px`）的正数。 | `"29.7cm"`    |
