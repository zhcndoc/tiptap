...
每个页眉和页脚槽位（`default`、`first`、`even`）都接受以下形态之一。您可以在各个槽位之间自由混用并搭配——选择最符合您数据的那一种。
...
Tiptap JSONContent 对象**必须**包含一个 `type` 字段——该扩展在运行时会使用它来区分 Tiptap 节点和 `docx` 实例。没有 `type` 且又不是 `Header` / `Footer` 实例的对象，会被丢弃并通过 `console.warn` 提示其具体值；它们绝不会被转发到 DOCX 序列化器。
...
前面三种形态可直接与 `@tiptap-pro/extension-export-pdf` 一起使用。`Header` / `Footer` 实例和异步工厂形态则需要安装 `@tiptap-pro/extension-export-docx`——它们会走优先 DOCX 的导出路径，并按需加载，因此不会影响仅使用 PDF 的消费者。
...
您可以直接在任意槽位上传入一个 Tiptap JSONContent 节点——当您已经在内存中拥有该节点，并且不想手动构建 `Docx.Header` 时，这会非常有用：
