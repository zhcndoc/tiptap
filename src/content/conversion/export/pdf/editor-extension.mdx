...
meta:
  ...
  description: 了解如何在我们的文档中使用导出 PDF 扩展将 Tiptap 编辑器内容导出为 PDF 文件。
  category: 转换
---
...
| `onExportError`    | `(error: Error) => void` | 导出流程被拒绝时调用的可选回调（动态导入失败、DOCX 转换失败、网络错误、非 OK 响应）。省略时，错误将通过 `console.error` 记录。 | `undefined`                           |
...
- **自定义字体是企业版 / 本地部署功能。** 在云计划中，携带 `customFonts` 的请求会返回 `403`。如需启用，请联系我们了解 [企业版计划](https://tiptap.dev/pricing) 或运行本地部署。
