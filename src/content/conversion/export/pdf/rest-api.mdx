...
<Callout title="接受的插槽值">
    ...
    - **字符串化的 Tiptap JSONContent** —— 可启用富文本格式（粗体、斜体、链接等）。可使用 `JSON.stringify(yourHeaderDoc)` 生成。
    - **Tiptap JSONContent 对象** —— 同样的富文本内容，作为嵌套 JSON 对象直接发送到请求体中（无需字符串化）。
...
| 422    | `FAILED_TO_EXPORT_PDF_FILE`    | 导出中间格式失败 |
