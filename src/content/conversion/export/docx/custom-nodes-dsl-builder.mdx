...
| 方法                     | 用途                                                                                                                                                                                                  |
| ------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `.applyMarks(policy)`    | `'node'`（继承该规则的 PM 节点 marks）或一个 `marks('node')...` 构建器，用于更细粒度控制。这里明确拒绝使用 `'default'` 和 `'none'` — 见 [标记策略](#mark-policies)。 |
...
- 当同一个 `type` 同时出现在两个 API 中时，基于函数的 [`customNodes`](/conversion/export/docx/custom-nodes) 会优先作用（每次导出调用中，每个冲突类型只记录一次警告）。
