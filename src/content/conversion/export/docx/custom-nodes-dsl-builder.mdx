...
包裹一个或多个 `TextRun` 子元素的行内元素。`link` 属性在类型层面是**必需的**，因此 `externalHyperlink({})` 会导致 TypeScript 错误。
...
`textRun().applyMarks(...)` 和 `externalHyperlink().applyMarks(...)` 仅接受 `'node'` 或 `marks('node')...` 构建器。这里的线格式会拒绝 `'default'` 和 `'none'`，因为它们只适用于子节点标记。
...
//                  ^^^^^^^^^^^^^^^^
// 类型“ChildrenBlockBuilder”不能赋给类型“ParagraphChildSpec”。

// Table.rows 需要 TableRow 构建器。
table().rows(paragraph())
//           ^^^^^^^^^^^
// 类型“ParagraphBuilder”不能赋给类型“TableRowBuilder”。

// TableRow 的子节点必须是 TableCell 构建器。
tableRow().children([tableRow()])
//                   ^^^^^^^^^^
// 类型“TableRowBuilder”不能赋给类型“TableCellBuilder”。
...
//                       ^^^^^^^^
// 类型“TextRunBuilder”上不存在属性“children”。

// externalHyperlink.link 是必需的。
externalHyperlink({})
//                ^^
// 类型“{}”中缺少属性“link”，但类型“ExternalHyperlinkProps”中要求该属性。

// applyMarks 拒绝 'default' / 'none'。
textRun().applyMarks('default')
//                   ^^^^^^^^^
// 类型“"default"”不能赋给类型“ApplyMarksPolicy | MarkPolicyBuilder”。
```
...
命名样式标签仍然有其作用：**往返一致性标识**。无论 `Hintbox` 是否在 `styles.xml` 中声明，DOCX 写入器都会生成 `<w:pStyle w:val="Hintbox"/>`。导入端会读取该 pStyle，以重建匹配的自定义节点。
...
这与 [`cssStyles.extractFromDocument`](/conversion/export/docx/css-to-docx) 配合得十分自然：让 CSS 处理标题、正文​​字体、列表和其他通用选择器；让 DSL 处理与自定义节点类型相关的内容。
...
- [自定义节点 DSL（JSON 参考）](/conversion/export/docx/custom-nodes-dsl)：wire format 的完整规范性描述。构建器只是其上的一层语法糖；有关运行时语义、错误代码和资源限制，请查阅参考文档。
- [基于函数的自定义节点](/conversion/export/docx/custom-nodes)：仅用于浏览器内自定义的 JavaScript 回调变体。
- [CSS 转 DOCX](/conversion/export/docx/css-to-docx)：与构建器自然配合，用于通用与自定义的分离。
- [样式覆盖](/conversion/export/docx/styles)：手动命名样式声明。对于希望向 Word 的样式选择器公开的字符/段落样式，它仍然有用；但仅仅用于装饰自定义节点时已不再需要。
