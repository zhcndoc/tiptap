...
该 REST API 支持 [样式覆盖](/conversion/export/docx/styles)、页面尺寸和边距、页眉/页脚，以及元素级覆盖（`tableOverrides`、`paragraphOverrides`、`textRunOverrides`、`tableCellOverrides`、`imageOverrides`）。它不支持自定义节点渲染，也不支持任何需要 JavaScript 函数的选项。如果您需要这些能力，请使用 [编辑器扩展](/conversion/export/docx/editor-extension)，它也适用于服务器端。
...
<Callout title="导出自定义支持" variant="info">
    `/v2/convert/export/docx` 端点接受 [样式覆盖](/conversion/export/docx/styles)、页面尺寸/边距、页眉/页脚，以及五种元素级覆盖（`tableOverrides`、`paragraphOverrides`、`textRunOverrides`、`tableCellOverrides`、`imageOverrides`）。它不支持自定义节点转换，因为这些功能需要无法序列化的 JavaScript 函数。对于自定义节点转换，请使用 [编辑器扩展](/conversion/export/docx/editor-extension) 或 [服务器端导出指南](/conversion/export/docx/editor-extension#server-side-export)。
</Callout>
...
<Callout title="需要订阅支持" variant="warning">
