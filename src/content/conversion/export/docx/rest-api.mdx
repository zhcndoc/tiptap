...
REST API 支持 [样式覆盖](/conversion/export/docx/styles)、页面尺寸和边距，以及页眉/页脚。它不支持自定义节点渲染、元素覆盖（`paragraphOverrides`、`textRunOverrides` 等），也不支持任何需要 JavaScript 函数的选项。如果您需要这些功能，请使用 [编辑器扩展](/conversion/export/docx/editor-extension)，它也可在服务端运行。
...
<Callout title="支持自定义导出" variant="info">
    `/v2/convert/export/docx` 端点不支持自定义节点转换或元素覆盖（`paragraphOverrides`、`textRunOverrides`、`tableOverrides`、`tableCellOverrides`、`imageOverrides`），因为这些需要无法序列化的 JavaScript 函数。它支持[自定义样式覆盖](/conversion/export/docx/styles)、页面尺寸/边距以及页眉/页脚。如果您需要自定义节点转换或元素覆盖，请使用[编辑器扩展](/conversion/export/docx/editor-extension)或[服务端导出指南](/conversion/export/docx/editor-extension#server-side-export)。
</Callout>
