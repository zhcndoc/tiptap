...
| 字段       | 类型                            | 默认值  | 描述                                                                                               |
| ---------- | ------------------------------- | -------- | -------------------------------------------------------------------------------------------------- |
| `type`     | `string`                        | 必填     | 此规则匹配的 PM 节点类型（例如 `"hintbox"` 或 `"mention"`）。                                      |
| `nodeKind` | `"block" \| "inline" \| "auto"` | `"auto"` | 源 PM 节点声明的类型。`"auto"` 允许验证器根据规则的发射形状推断类型。                             |
| `render`   | `RenderProgram \| null`         | 必填     | 要渲染的内容。`null` 将删除匹配的 PM 节点及其所有**后代节点**。                                  |

如果希望包装节点不输出任何内容，但仍正常渲染其子节点，请使用 `render: { emit: { "$children": { "as": "block" } } }`，而不是 `render: null`。
