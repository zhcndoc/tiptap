...
1. **普通文本页眉和页脚中的 `{page}` / `{total}` 标记**：当内容来自 [Pages 扩展](/pages/getting-started/overview) 自动提取时，以及通过 `headers` / `footers` 传入普通文本字符串时，都会自动生效。导出时会将每个标记转换为实时的 `PAGE` / `NUMPAGES` 字段，因此 Word 会显示实际的当前页码和总页数。
...
如果你的编辑器通过 `Pages.configure({ placeholders: { total: 'pages' } })` 重命名了内置标记（参见 [Pages → 重命名占位符](/pages/core-concepts/page-header-footer#renaming-or-disabling-placeholders)），导出也会遵循相同的标记名称：`{pages}` 会变成实时的 `NUMPAGES` 字段，而原始的 `{total}` 将不再被识别。`ExportDocx` 本身没有 `placeholders` 选项；在 Pages 上进行配置可保持编辑器预览与导出的 `.docx` 一致。
