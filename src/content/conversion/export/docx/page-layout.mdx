...
| 属性 | 类型 | 描述 | 默认值 |
|----------|------|-------------|---------|
| `top` | `UniversalMeasure` | 页面的上边距。<br />可以为负值。 | `"1.0cm"` |
| `bottom` | `UniversalMeasure` | 页面的下边距。<br />可以为负值。 | `"1.0cm"` |
| `left` | `PositiveUniversalMeasure` | 页面的左边距。必须为正值。 | `"1.0cm"` |
| `right` | `PositiveUniversalMeasure` | 页面的右边距。必须为正值。 | `"1.0cm"` |
| `header` | `PositiveUniversalMeasure` | 页面顶部边缘到页眉顶部的距离。应小于 `top`，以便页眉位于正文内容上方。 | Word 默认值 `"1.25cm"` |
| `footer` | `PositiveUniversalMeasure` | 页面底部边缘到底部页脚的距离。应小于 `bottom`，以便页脚位于正文内容下方。 | Word 默认值 `"1.25cm"` |
...
<Callout title="Auto-derived from the Pages extension" variant="hint">
    当 [Pages 扩展](/editor/extensions/pages) 与 `ExportDocx` 一同配置时，`header` 和 `footer` 偏移会自动根据 Pages 存储值 `headerTopMargin` 和 `footerBottomMargin` 生成。您传入 `pageMargins` 的值始终具有优先级——如需覆盖自动生成的值，请显式设置 `header` 或 `footer`。
</Callout>
