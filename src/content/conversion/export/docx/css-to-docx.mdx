...
1. **收集 CSS。** 导出器会从两个可能的来源读取你的样式（你可以任选其一，也可以同时使用两者）：
   - 你在 `configure()` 中传入的显式 `styles` 对象
   - 浏览器的实时样式表（如果 `extractFromDocument: true`），作用域限定在你的编辑器根节点（editor root）
2. **编译每个选择器。** 编译器会在一个固定映射（导入端相同的 16 个选择器）中查找每个选择器，将每个 CSS 属性转换为对应的 DOCX 属性，并丢弃无法映射的属性。
3. **合并到样式目录（styles catalog）。** 编译后的 DOCX 样式片段会合并到导出选项的样式目录中，然后交给 DOCX 写入器（writer）。
...
ExportDocx.configure({
  cssStyles: {
    ...
    // 可选：用于解析 rem/em 的基准字体大小
    baseFontSize: 16,
  },
})
```

当 `extractFromDocument: true` 时，导出器会遍历 `document.styleSheets`，查找作用域位于你的编辑器根节点内、且匹配这 16 个选择器映射的规则，并将它们与显式传入的 `styles` 合并。**对于每个属性，后者/显式样式优先（逐属性生效）。**
...
- [CSS 注入（导入）](/conversion/import/docx/css-injection)：反向方向。
- [导出样式](/conversion/export/docx/styles)：原始的 DOCX 导出样式指南（手动样式定义，不基于 CSS）。
