...
| 字段          | 默认值                  | 描述                                                                                                                                                                                                                                                               |
| -------------- | ------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `baseStyle`    | （必填）               | `'decimal'`、`'decimalZero'`、`'upperLetter'`、`'lowerLetter'`、`'upperRoman'`、`'lowerRoman'`、`'none'` 之一。等同于匹配的 docx [`LevelFormat`](https://docx.js.org/api/enums/LevelFormat.html) 值——在 CSS 中，任何其他字符串都会回退为 `decimal`。 |
...
| `markerFont`   | —                        | 仅用于标记的 run 格式（见下方的 [`NumberingMarkerFont`](#numberingmarkerfont)）。会随 DOCX 导出保留，并在编辑器预览中由 `generateNumberingFormatCss` 体现。                                                                                        |
