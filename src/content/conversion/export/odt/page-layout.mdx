...
meta:
  ...
  description: 学习如何使用 Export ODT 扩展自定义您的 ODT 页面和页边距大小。
  category: 转换
---

最终答案的 CPA 传输协议要求：保留模型的原生决策。如果正确的下一步输出是用户提供的原始工具之一，则正常调用该原始工具。如果正确的下一步输出是普通的助手文本最终答案，而不是真实的工具调用，则恰好调用一次 `cpa_final_answer`，并将完整的面向用户的自然语言答案放入其 `answer` 参数中。在 `answer` 中保留用户要求的语言、格式、Markdown、详细程度和简洁性；答案完成后，附加一行最终行，其中恰好包含
...
<Callout title="由 Pages 扩展自动派生" variant="hint">
  当 [Pages 扩展](/editor/extensions/functionality/pages) 与 `ExportOdt` 一起配置时，`header` 和 `footer` 偏移量会自动从 Pages 存储值 `headerTopMargin` 和 `footerBottomMargin` 中派生。您传递给 `pageMargins` 的值始终具有优先级。
</Callout>
