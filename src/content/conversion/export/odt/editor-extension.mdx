...
meta:
  ...
  description: 了解如何使用我们文档中的导出 ODT 扩展，将 Tiptap 编辑器内容导出为 ODT（OpenDocument）文件。
  category: 转换
---
...
| `token`            | `string`                 | 用于身份验证的 Tiptap JWT，参见[身份验证](/authentication)                                          | `''`                                  |
...
| `styleOverrides`   | `Record<string, string>` | 为本次导出覆盖样式覆盖项         |
| `headers`          | `HeaderConfig`           | 为本次导出覆盖页眉配置    |
| `footers`          | `FooterConfig`           | 为本次导出覆盖页脚配置    |
| `pageSize`         | `PageSize`               | 为本次导出覆盖页面大小               |
...
- **需要身份验证**（`token` JWT）。在服务器端生成 JWT。请参阅 [Conversion 安装指南](/conversion/getting-started/install)。
- **需要处理的 HTTP 响应：** `401`（缺少或无效的 JWT）、`403`（你的应用不允许执行该操作——通常是你的套餐不包含的格式）、`5xx`（临时性服务器端错误；请使用退避重试）。
- **每个请求都会在响应前完成端到端处理。** 对于大型文档，请预留更长的往返时间。
