...
<Requirements>
    ...
    <RequirementItem label="2. 配置 Convert 应用">
        使用携带 `aud: "Convert"` 的签名令牌验证 REST 调用。参见 [认证](/authentication)。
    </RequirementItem>
</Requirements>
...
| `headerFooterOverrides` | `Object` | 针对页眉/页脚内容的按字段覆盖，作用域限定于页眉/页脚内容（与元素覆盖相同的五个键）。参见 [元素覆盖](#element-overrides)。 | `undefined` |
| `placeholders`       | `Object \| false` | 用于页眉/页脚中页码标记的重命名映射（`{ page?, total? }`），与 `Pages.configure({ placeholders })` 匹配。传入 `false` 可禁用替换。参见 [自定义标记名称](/conversion/export/docx/rest-api#custom-token-names-placeholders)。 | `undefined` |
