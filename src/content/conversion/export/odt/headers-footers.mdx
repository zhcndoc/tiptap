...
| `default`             | `string`   | 所有页的默认页眉，或当激活 `evenAndOddHeaders` 时，奇数页使用的页眉。支持纯文本字符串或字符串化的 Tiptap JSONContent 以实现富格式化。  |
| `first`               | `string`   | 首页页眉，仅当 `differentFirstPage` 设为 `true` 时生效。支持纯文本字符串或字符串化的 Tiptap JSONContent 以实现富格式化。            |
| `even`                | `string`   | 偶数页页眉，仅当激活 `evenAndOddHeaders` 时生效。支持纯文本字符串或字符串化的 Tiptap JSONContent 以实现富格式化。                    |
