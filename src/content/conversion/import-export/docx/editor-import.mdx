...
<Requirements>
    <RequirementItem label="1. 激活试用或订阅">
        在你的账户中开始免费[试用](https://cloud.tiptap.dev/v2/billing)或订阅[Tiptap 计划](https://cloud.tiptap.dev/v2/billing)。
    </RequirementItem>
    <RequirementItem label="2. 从私有注册表安装">
        要安装此前端扩展，请按照[设置指南](/guides/pro-extensions)的步骤验证 Tiptap 的私有 npm 注册表。
    </RequirementItem>
</Requirements>
...
如果需要在编辑器外部导入 `.docx` 内容，请使用[REST API](/conversion/import-export/docx/rest-api)。

<Callout title="集成或测试转换" variant="warning">
    要测试或集成此功能，你需要一个[活动的试用或 Tiptap 订阅](https://cloud.tiptap.dev/v2/billing)。
</Callout>
