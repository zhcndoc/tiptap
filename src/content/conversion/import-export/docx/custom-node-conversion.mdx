...
`@tiptap-pro/extension-import` 的最新版本提供了 `promisemirrorNodes` 配置选项。此选项允许您将 DOCX 中的自定义节点映射到您的 Tiptap 模式。在上面的示例中，我们将 DOCX 中的 `Hintbox` 自定义节点映射到我们 Tiptap 模式中的 `hintbox` 自定义节点。这样，每当 DOCX 包含 `Hintbox` 自定义节点时，它将被转换为 Tiptap 中的 `hintbox` 节点。
