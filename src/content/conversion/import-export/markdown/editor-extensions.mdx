...
<Requirements>
    <RequirementItem label="1. 激活试用或订阅">
        在您的账户中启动免费 [试用](https://cloud.tiptap.dev/v2/billing) 或订阅 [Tiptap 计划](https://cloud.tiptap.dev/v2/billing)。
    </RequirementItem>
...
<Callout title="集成或测试转换功能" variant="warning">
    若要测试或集成此功能，您需要一个 [激活的试用或 Tiptap 订阅](https://cloud.tiptap.dev/v2/billing)。
</Callout>
...
```
此操作会将所选的 `.md` 文件上传到转换API，将其转换为 Tiptap JSON，并替换当前编辑器内容。
