---
title: 通过 REST API 转换 Markdown（旧版）
tags:
  - type: deprecated
meta:
    title: 旧版 Markdown REST API | Tiptap 转换
    description: 了解如何通过旧版 REST API 进行 Tiptap Markdown 文件的转换。
    category: 转换
---
...
<Callout title="旧版 Markdown REST API（已弃用）" variant="warning">
    此 REST API 已弃用，并将在 2026 年的某个时间点停止服务，团队会提前通知。对于新的集成，请使用更新后的 [Markdown 导出 REST API](/conversion/export/markdown/rest-api) 和 [Markdown 导入 REST API](/conversion/import/markdown/rest-api)。
</Callout>
