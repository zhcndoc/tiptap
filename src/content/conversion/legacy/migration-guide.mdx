...
- **更好的 DOCX 保真度。** 当前的 DOCX 路径会保留旧扩展会丢失的属性（段落间距、图片裁剪、表格单元格属性）。可配合 [ConvertKit](/conversion/import/docx/convertkit) 在编辑器中渲染这些属性。
...
ConvertKit 是一个单独安装的包；它不包含在 import 或 export 扩展中。有关它会注册哪些内容，请参阅 [ConvertKit 参考文档](/conversion/import/docx/convertkit)。

如果你还在使用 [Pages 扩展](/pages/getting-started/overview)，迁移后的配置还需要添加来自 `@tiptap-pro/extension-pages-tablekit` 的 `TableKit`，并禁用 ConvertKit 的表格功能（因为这些表格不适合分页）：
...
- [转换概览](/conversion/getting-started/overview): 当前产品
- [安装](/conversion/getting-started/install): 凭证和注册表设置
- [ConvertKit](/conversion/import/docx/convertkit): 你将在编辑器上注册的 schema
- [端到端演练](/conversion/getting-started/guides/end-to-end-walkthrough): 使用新 API 完成完整的 DOCX 往返流程
- [旧版概览](/conversion/legacy/overview): 供参考，旧版包目前仍在做什么
