...
使用带有 `Documents:Api:All` 权限的签名 token 进行身份验证，并作为 `Authorization: Bearer <jwt>` 标头发送。有关如何签名，请参见 [身份验证](/authentication)。此 token 授予完整的 Document Server API 访问权限，因此请将其保留在服务器端。先前的 API secret 仍然可用，并在 [传统身份验证](/authentication/legacy) 中有相关文档。
...
<Callout title="创建线程是一个两步过程" variant="info">
  调用此端点会创建线程，但不会将其附加到文档中的任何内容。要使线程在编辑器中显示，您还需要在文档的 Tiptap JSON 中使用 `inlineThread` 标记包裹目标范围。完整工作流程请参见 [通过 API 创建线程指南](/comments/integrate/create-thread-via-api)。
</Callout>
