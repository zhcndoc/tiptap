...
<Callout title="最佳实践" variant="info">
  早期在设置中使用 Pages 扩展，确保你的文档结构和布局从一开始就是一致的。
</Callout>
...
<Callout title="提示">
  <code>onCompleteExport</code> 回调是必需的。这是你处理导出文件的地方（例如，下载、上传等）。
</Callout>
...
<Callout title="提示">
  DOCX 导出扩展与 Pages 扩展无缝协作。请注意，Pages 扩展与 Microsoft Word DOCX 文件之间并非完美的 1:1 映射，因为渲染引擎完全不同。
</Callout>
...
<Callout title="提示">
  你需要有效的 <code>appId</code> 和 <code>token</code> 才能使用 ImportDocx 扩展。有关设置，请参阅 [Pro 扩展指南](/guides/pro-extensions)。
</Callout>
...
- 探索更多的 [Pages 选项](/pages/core-concepts/options)，以微调你的布局。
- 使用 Pages 扩展中包含的 Table 扩展添加 [表格](/pages/core-concepts/limitations#tables)，以获得最佳兼容性。
- 结合其他 Tiptap Pro 扩展，实现功能丰富的编辑器。

<Callout title="需要帮助？">
  查看其余文档，或联系我们的支持团队，获取更高级的工作流帮助！
</Callout>
