...
<Callout title="为什么是这三个包" variant="info">
  ...
  登录到 Tiptap 私有 npm registry；设置方法请参见 [Pro Extensions 指南](/guides/pro-extensions)。
</Callout>
...
DOCX 导出扩展完全在浏览器中运行：无需令牌，也无需调用服务器。你可以通过 `onCompleteExport` 回调进行配置，该回调会接收导出结果：
...
<Callout title="获取你的 token" variant="hint">
  参见 [转换安装指南](/conversion/getting-started/install) 进行设置，以及
  [身份验证](/authentication) 了解如何签名 token。请在服务端签名 token，且永远不要
  在浏览器中暴露你的签名密钥。
</Callout>
...
- [页面页眉和页脚](/pages/core-concepts/page-header-footer)：不同的首页、奇偶页、以编程方式打开/关闭编辑器
- [PagesTableKit](/pages/guides/pages-tablekit)：详细介绍可分页安全的表格，包括最重要的已知限制
- [限制](/pages/core-concepts/limitations)：不可拆分内容、Pages 不做什么，以及如何在每种情况下规避
