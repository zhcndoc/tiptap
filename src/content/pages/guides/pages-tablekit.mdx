...
<Requirements>
  ...
  <RequirementItem label="2. 从私有仓库安装">
    请按照 [设置指南](/guides/pro-tables) 完成验证，以访问 Tiptap 的私有 npm 仓库。
  </RequirementItem>
</Requirements>

## 哪种设置适合我？

<Callout title="两种场景，同一套编辑器堆栈" variant="info">
  PagesTableKit 在这两种场景中都很有用，区别在于表格内容如何进入编辑器，而不是如何配置编辑器。基础扩展相同；第二种场景会在此基础上添加 Import 扩展。
</Callout>

- **仅 Pages 工作流。** 你正在构建一个分页编辑器（适合打印的布局、信件样式的文档、报告），并且需要表格能够正确分页。内容由手动输入。
- **Pages + DOCX 导入工作流。** 你正在导入 `.docx` 文件，并在分页编辑器中渲染这些文件。你需要表格能够分页，**并且忠实反映原始 DOCX 的格式**（声明的宽度、行高、缩进）。

两种场景都使用相同的编辑器堆栈：`ConvertKit`（用于架构）+ `TableKit`（用于支持安全分页的表格）+ `Pages`（用于布局）。场景 B 会额外添加 `ImportDocx`。下面的每个部分都会说明其适用的场景。
...
每个打包的扩展都可以通过 `TableKit.configure()` 进行配置（或设置为 `false` 以禁用）：
...
- [页面概览](/pages/getting-started/overview)：PagesTableKit 所需的分页扩展。
- [表格中的页面](/pages/guides/table-with-pages)：在 Pages 中编写表格的一般指南。
- [ConvertKit](/conversion/import/docx/convertkit)：支持 DOCX 的编辑器套件；用于场景 B。
- [编辑器扩展（导入 DOCX）](/conversion/import/docx/editor-extension)：`ImportDocx` 扩展如何将 Word 内容导入你的编辑器。
- [CSS 注入（导入）](/conversion/import/docx/css-injection)：将 DOCX 样式目录提取为 CSS，以便在分页内容中保持一致的排版。
