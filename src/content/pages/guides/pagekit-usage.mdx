...
PageKit 是一个便捷扩展，它将 `Pages`、分页安全的 `TableKit` 以及 `PageBreak` 封装到一次注册中，这样你就可以在同一处进行配置。默认情况下，**只会注册 Pages 和 PageBreak**。`TableKit` 需要显式启用：传入任意选项对象（例如 `table: {}`）即可启用它。PageKit **不包含** `ConvertKit`；ConvertKit 是一个单独安装的包，需要与 PageKit 配合使用。
