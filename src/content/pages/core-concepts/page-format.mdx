...
Pages 使用标准 CSS 像素（1 英寸等于 96 像素），因此下面的尺寸与浏览器实际渲染的结果一致。Pages 扩展会在内部保留小数像素值，以避免累计的布局偏移；此处显示的整数已为便于阅读而四舍五入。

| 名称      | 尺寸（cm）      | 尺寸（px）      | 边距（cm：上、右、下、左）               | 边距（px）     |
| --------- | --------------- | --------------- | -------------------------------------- | -------------- |
...
每种内置格式都附带默认的边距和尺寸。若要自定义它们，请定义你自己的格式对象。另见下面的 [自定义页面格式](#custom-page-formats)。
...
## Programmatically change page format

After the editor is initialized, use the `setPageFormat` command to change the page format:

```js
// Switch to built-in format
