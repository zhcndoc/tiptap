...
<Callout title="首页槽位在启用时保持为空" variant="info">
...
<Callout title="启用时奇数页槽位会继承默认值" variant="info">
...
<Callout title="空的特定类型槽位仍会渲染为空" variant="info">
...
<Callout title="协作使用回调形式" variant="info">
  `headerFooterExtensions` 也接受一个 `(ctx) => Extensions` 回调，该回调会针对每个页眉/页脚子类型触发一次，并传入子编辑器应绑定的 Y 字段名和 Y.Doc。在配置协作功能时请使用这种形式——请参见 [为 Pages 添加协作](/pages/guides/collaboration-with-pages)。
</Callout>
...
将 `editableHeader` 或 `editableFooter` 设为 `false`，即可在不提供双击编辑入口的情况下渲染页眉/页脚。锁定后，用户或代码都无法打开覆盖层——`cursor: pointer` 提示会被移除，`openHeaderEditor` / `openFooterEditor` 命令会返回 `false`，而任何已打开的覆盖层都会被关闭（从而恢复主编辑器的 `editable` 状态）。
...
<Callout title="锁定不会隐藏内容" variant="info">
  锁定时页眉和页脚仍会正常渲染——只是移除了行内编辑入口。如果你想清空内容，请使用 `setHeader('')` / `setFooter('')`。
</Callout>
...
页眉和页脚编辑器会与主文档一起参与协作。每个子类型（默认、首页、奇数页、偶数页）都独立协作，`differentFirstPage` / `differentOddEven` 开关会在各客户端之间同步，文档级页面几何信息（`pageFormat`、`pageGap`、`headerTopMargin`、`footerBottomMargin`）也会同步——因此远程的 `setPageFormat` 或边距变更会在每个客户端上重新排版。要启用协作，请将 `headerFooterExtensions` 作为一个**回调**传入，为每个子编辑器附加 `Collaboration` 扩展；静态数组形式不会为子编辑器启用协作。完整设置流程请参阅 [为 Pages 添加协作](/pages/guides/collaboration-with-pages)。

<Callout title="协作时省略页眉/页脚默认值" variant="warning">
  当协作开启时，不要在 `Pages.configure()` 中传入 `header` / `footer` 默认值——在其他客户端清空内容后，它们可能会重新出现。请将默认值留空；如果你需要一个初始值，请在 provider 同步完成后通过 `editor.commands.setHeader(...)` 只设置一次。
</Callout>
...
| `footerBottomMargin`                 | `number`                          | 底部边距的 50%        | 从页脚内容到底部页面边缘的距离                                              |
...
| `differentOddEven`                   | `boolean`                         | `false`              | 启用不同的奇偶页页眉和页脚                                                   |
...
| `editableHeader`                     | `boolean`                         | `true`               | 双击页眉时是否打开内联编辑器                                                 |
| `editableFooter`                     | `boolean`                         | `true`               | 双击页脚时是否打开内联编辑器                                                 |
| `headerFooterExtensions`             | `Extensions \| (ctx) => Extensions` | `ConvertKit`         | 页眉/页脚子编辑器的扩展。非协作模式下传入静态数组；要为每个子类型附加 `Collaboration`，请传入回调——参见 [为 Pages 添加协作](/pages/guides/collaboration-with-pages)。 |
| `onDblClickHeaderFooterPreventClose` | `(event: MouseEvent) => boolean` | `undefined`          | 防止在外部双击时关闭页眉/页脚编辑器的回调                                     |
| `onDblClickHeaderPreventClose`       | `(event: MouseEvent) => boolean` | `undefined`          | 防止在外部双击时关闭页眉编辑器的回调                                         |
| `onDblClickFooterPreventClose`       | `(event: MouseEvent) => boolean` | `undefined`          | 防止在外部双击时关闭页脚编辑器的回调                                         |
...
| `placeholders`                       | `false \| { page?, total? }`      | `undefined`          | 重命名内置 `{page}` / `{total}` 标记，或禁用替换。                           |
