...
| `headerHeight`      | `number` (px)                | `50`      | 页眉高度，单位为像素                                                                        |
| `footerHeight`      | `number` (px)                | `50`      | 页脚高度，单位为像素                                                                        |
| `pageGap`           | `number` (px)                | `50`      | 页面间距，单位为像素                                                                        |
| `header`            | `string \| fn`               | `''`      | 页眉内容；字符串（支持 `{page}`/`{total}`）或函数 `(pageNumber, totalPages) => string`     |
| `footer`            | `string \| fn`               | `'{page}'`| 页脚内容；字符串（支持 `{page}`/`{total}`）或函数 `(pageNumber, totalPages) => string`     |
| `pageGapBackground` | `string` (CSS color)         | `undefined`| 页面间距的背景颜色                                                                         |
| `onPageFormatChange`| `fn`                         | `() => {}`| 页面格式变化时的回调函数；接收新的 PageFormat                                               |
