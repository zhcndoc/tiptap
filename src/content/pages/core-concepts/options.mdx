...
本页面是 Pages 扩展的 **选项**、**命令** 和 **存储** 的权威参考。有关面向任务的演练，请参见 [Page format](/pages/core-concepts/page-format)、[Page header and footer](/pages/core-concepts/page-header-footer)、[Page gap](/pages/core-concepts/page-gap)、[Page break node](/pages/core-concepts/page-break-node) 和 [Zoom](/pages/core-concepts/zoom)。
...
| `headerTopMargin`                    | `number`                         | 上边距的 50%    | 从页面顶部到页眉内容的距离（px）             |
| `footerBottomMargin`                 | `number`                         | 下边距的 50% | 从页脚内容到底部页面的距离（px）          |
...
| `differentOddEven`                   | `boolean`                        | `false`              | 奇数页和偶数页具有不同的页眉和页脚         |
...
| `editableHeader`                     | `boolean`                        | `true`               | 允许在双击时打开页眉覆盖层          |
| `editableFooter`                     | `boolean`                        | `true`               | 允许在双击时打开页脚覆盖层          |
| `headerFooterExtensions`             | `Extensions \| (ctx) => Extensions` | `ConvertKit`         | 内部页眉/页脚编辑器的扩展。传入回调可针对每个子类型附加 `Collaboration`（参见 [为 Pages 添加协作](/pages/guides/collaboration-with-pages)）。 |
...
| `setDifferentFirstPage`       | `enabled: boolean`          | 切换首页页眉和页脚不同     |
| `setDifferentFirstPageFooter` | `enabled: boolean`          | 独立切换首页页脚不同    |
| `setHeaderFirstPage`          | `header: PagesHeaderFooter` | 设置首页页眉内容                       |
| `setFooterFirstPage`          | `footer: PagesHeaderFooter` | 设置首页页脚内容                       |
| `setDifferentOddEven`         | `enabled: boolean`          | 切换奇偶页页眉和页脚不同     |
| `setDifferentOddEvenFooter`   | `enabled: boolean`          | 独立切换奇偶页页脚不同     |
...
| `openHeaderEditor`            | `{ pageNumber: number }`    | 打开指定页面的页眉编辑器（从 1 开始编号）。 |
| `openFooterEditor`            | `{ pageNumber: number }`    | 打开指定页面的页脚编辑器（从 1 开始编号）。 |
| `closeHeaderFooterEditors`    | none                        | 关闭当前打开的任一页眉/页脚编辑器        |
| `closeHeaderEditor`           | none                        | 如果已打开，则关闭页眉编辑器                     |
| `closeFooterEditor`           | none                        | 如果已打开，则关闭页脚编辑器                     |
