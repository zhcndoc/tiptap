...
interface PagesOptions {
  ...
  /** 注册到内部页眉/页脚编辑器上的扩展。
   *
   *  接受静态 `Extensions` 数组，或者接受一个回调函数：该函数会针对每个子类型（默认 / 首页 / 奇数页 / 偶数页 ×
   *  页眉 / 页脚）接收一个 `HeaderFooterExtensionsContext`，并返回该子编辑器的扩展。使用
   *  回调形式可针对每个子类型附加 `Collaboration` 到自动计算的 Y 字段上——参见为 Pages 添加协作。
   *
  ...
  /** 类 Word 的脚注。除非 `enabled` 为 true，否则禁用——参见脚注。 */
  footnotes?: {
    /** 主开关。默认：false。 */
    enabled?: boolean
...
| `headerTopMargin`                    | `number`                            | 页面上边距的 50%    | 从页面顶部到页眉内容的距离（px）                                                                                                                                        |
| `footerBottomMargin`                 | `number`                            | 页面下边距的 50% | 从页脚内容到底部页面的距离（px）                                                                                                                                     |
| `differentFirstPage`                 | `boolean`                           | `false`              | 首页的页眉和页脚与其余页面不同                                                                                                                                    |
| `headerFirstPage`                    | `PagesHeaderFooter`                 | `''`                 | 首页页眉内容                                                                                                                                                            |
| `footerFirstPage`                    | `PagesHeaderFooter`                 | `''`                 | 首页页脚内容                                                                                                                                                            |
| `differentOddEven`                   | `boolean`                           | `false`              | 奇数页/偶数页有不同的页眉和页脚                                                                                                                                    |
...
| `editableHeader`                     | `boolean`                           | `true`               | 允许双击打开页眉覆盖层                                                                                                                                     |
| `editableFooter`                     | `boolean`                           | `true`               | 允许双击打开页脚覆盖层                                                                                                                                     |
| `headerFooterExtensions`             | `Extensions \| (ctx) => Extensions` | `ConvertKit`         | 内部页眉/页脚编辑器的扩展。传入回调可按子类型附加 `Collaboration`（参见 [为 Pages 添加协作](/pages/guides/collaboration-with-pages)）。 |
...
| `placeholders`                       | `false \| { page?, total? }`        | `undefined`          | 重命名 `{page}` / `{total}` 标记，或使用 `false` 禁用                                                                                                                            |

### 脚注

脚注选项的完整参考位于 [Footnotes](/pages/core-concepts/footnotes#complete-options-reference)。以下所有选项都位于 `footnotes` 键下，除非另有说明。
...
| `differentOddEven`         | `boolean`                     | 奇数页/偶数页是否不同                                                  |
| `differentFirstPageFooter` | `boolean`                     | 首页页脚是否不同（独立标志）                                   |
| `differentOddEvenFooter`   | `boolean`                     | 奇数/偶数页脚是否不同（独立标志）                                     |
...
| `footnoteNumbers`   | `Record<string, number>`      | 当前编号（note id → 从 1 开始的编号）                         |
| `footnotesEnabled`  | `boolean`                     | 脚注是否已启用                                        |
| `editableFootnotes` | `boolean`                     | 是否已解锁编辑（只读——使用 `setFootnotesEditable`） |

### 活动的内部编辑器状态

当页眉、页脚或脚注覆盖层打开时，这些属性会反映当前激活的是哪一个。对于构建跟随当前聚焦编辑器的工具栏很有用——完整的工具栏示例请参见 [页面页眉和页脚](/pages/core-concepts/page-header-footer#active-editor-state)。

| Property             | Type                                          | Description                                                          |
| -------------------- | --------------------------------------------- | -------------------------------------------------------------------- |
| `activeEditor`       | `Editor \| null`                              | 当前激活的页眉/页脚/脚注编辑器实例，或 `null`        |
| `activeEditorType`   | `'header' \| 'footer' \| 'footnotes' \| null` | 当前激活的是哪种覆盖层                                      |
| `activePageNumber`   | `number \| null`                              | 正在编辑的页码                                             |
| `headerEditorOn`     | `Editor['on'] \| null`                        | 预绑定的页眉覆盖层内部编辑器事件订阅器 |
| `headerEditorOff`    | `Editor['off'] \| null`                       | 预绑定的页眉覆盖层内部编辑器取消订阅器         |
| `footerEditorOn`     | `Editor['on'] \| null`                        | 预绑定的页脚覆盖层内部编辑器事件订阅器 |
| `footerEditorOff`    | `Editor['off'] \| null`                       | 预绑定的页脚覆盖层内部编辑器取消订阅器         |
