...
当同时注册 PageBreak 和 Pages 时，PageBreak 会自动以 Pages 模式运行。使用标准的 [Pages 栈](/pages/getting-started/install) —— ConvertKit + TableKit + Pages —— 并在其中添加 PageBreak：
...
Tiptap 的 [conversion service](/conversion/import/docx/editor-extension) 在导入包含分页符的 `.docx` 文件时会生成 `pageBreak` 节点。这些节点会直接映射到此扩展的节点类型，因此导入的文档会保留其分页符位置。
