...
meta:
  ...
  description: 向 Tiptap Pages 添加类似 Word 的脚注——按页脚注区域、自动编号、就地编辑以及 DOCX 往返转换。
  category: Pages
sidebars:
  hideSecondary: true
---
...
脚注让读者可以用编号注释文本，这些注释会显示在页面底部——就在页脚上方，和 Microsoft Word 中完全一样。每个脚注都锚定到正文中的一个上标参考标记。脚注区域会占用页面可用空间的一部分，因此一页中的脚注越多，正文可用空间就越少。

脚注始终与其引用保持在一起：如果编辑导致引用文本移到另一页，它的脚注会自动移动到那一页的脚注区域。

<Callout title="从 DOCX 导入" variant="hint">
  当你使用 [DOCX 导入扩展](/conversion/import/docx/editor-extension#footnotes--endnotes) 导入 `.docx` 文件时，文档中的脚注会自动应用到 Pages——包括引用、内容和编号。无需额外配置。
</Callout>
...
你只需要这些——脚注引用节点会自动注册，只要文档中包含脚注，各页的脚注区域就会立即渲染。

## 脚注的行为方式

其思维模型与 Microsoft Word 一致：

- **一个标记，一条注释。** 每个脚注都是正文中的一个上标数字，与页面脚注区域中的一个编号条目配对。
- **脚注位于页脚上方。** 每一页都会渲染自己的脚注区域，并带有一条简短的分隔线，位于正文内容和页面页脚之间。
- **脚注会占用页面空间。** 该区域会随着内容增大而扩展，相应地正文区域会缩小——在满页中增加脚注会把正文推到下一页。
- **脚注会跟随其引用。** 该区域只显示位于该页上的脚注标记对应的脚注。文本跨页重排时，脚注也会随之移动；你无需手动管理脚注位置。
- **编号自动且连续。** 脚注会按文档顺序编号为 `1, 2, 3…`。在两个已有脚注之间插入新的脚注会重新编号其后的所有脚注；删除一个脚注会补上空缺。编号是计算得出的，不会被存储——始终准确。
...
可通过 <kbd>Escape</kbd>、关闭按钮，或在其外部双击来关闭编辑器。当脚注编辑器打开时，主文档编辑器会暂时不可编辑——行为与页眉和页脚编辑器相同。
...
<Callout title="协作使用回调形式" variant="info">
  `footnotes.extensions` 也接受 `(ctx) => Extensions` 回调，该回调会接收脚注编辑器应绑定到的 Y 字段名称和 Y.Doc。在配置协作时请使用这种形式——见下方 [脚注与协作](#footnotes-and-collaboration)。
</Callout>

### 活动编辑器状态

当脚注编辑器打开时，该扩展会通过 storage 暴露它——与页眉和页脚采用相同模式，因此统一工具栏可以同时适用于三者：
...
如需查看一个完整的自定义工具栏示例，让其跟随当前聚焦的编辑器，请参见 [页面页眉和页脚 → 构建自定义工具栏](/pages/core-concepts/page-header-footer#building-a-custom-toolbar)——为其添加脚注支持只需处理 `activeEditorType` 额外的 `'footnotes'` 值。
...
锁定后，脚注仍会正常渲染——只是无法编辑。`openFootnoteEditor` 会返回 `false`，双击不会有任何反应，且已打开的脚注编辑器会被关闭。

### 防止双击时关闭

与页眉和页脚一样，当用户在脚注编辑器外部双击时，你可以让脚注编辑器保持打开——当你自己的工具栏位于编辑器外部时尤其有用：
...
从正文中删除引用标记会立即将其脚注从页面中移除——并重新编号其余脚注。脚注的**内容会在后台保留**，这样普通撤销就能同时恢复标记和其文本。这一点有意不同于 Word（Word 会立即删除内容），以优先保障撤销安全性。
...
引用标记总会生成一个可见的脚注——即使它当前还没有任何内容（例如在 `setContent()` 之后，文档中包含标记但尚未提供任何脚注内容时）。这类脚注会显示为空的编号条目，而打开脚注编辑器后就可以立即编辑它们。

## 从标记导航

点击正文中的脚注标记会滚动页面，使其脚注可见——在长文档中很方便。
...
`initialContent` 接受以注释 id 为键的脚注内容——每个 id 都对应文档内容中 `footnoteReference` 节点的 `noteId` 属性。其值为 Tiptap 的 `JSONContent` 文档：
...
<Callout title="协作" variant="info">
  `initialContent` 仅在未启用协作时生效——在协作文档中，共享文档拥有脚注内容。若要在协作文档中作为显式用户操作加载脚注（例如在 DOCX 导入后），请改用 `setFootnotes` 命令。
</Callout>
...
- **导入** — 使用 [DOCX 导入编辑器扩展](/conversion/import/docx/editor-extension#footnotes--endnotes) 时，导入文件中的脚注会自动应用：标记会出现在正文中，内容会落入页面区域，编号与源文档一致。如果你希望自行处理，导入上下文还会暴露原始的 `footnotes` / `endnotes` 数据。
- **导出** — 使用 [DOCX 导出编辑器扩展](/conversion/export/docx/editor-extension) 时，脚注内容会自动从 Pages 中提取并写为真正的 Word 脚注：标记会变成可用的脚注引用，Word 会原生渲染并重新编号。

<CodeDemo isPro path="/Extensions/PagesFootnotesImportExportDocx" />

任一侧都无需配置——只需将导入/导出扩展与 Pages 一起安装，脚注就会被包含在内。

## 脚注与协作

脚注会与主文档一起参与协作：并发用户可以实时看到彼此对脚注所做的编辑，插入或删除脚注也会在各个客户端之间收敛一致，包括编号。

要启用此功能，请将 `footnotes.extensions` 作为一个**回调**传入，用于向脚注编辑器附加 `Collaboration` 扩展——页眉和页脚使用的也是同样的模式：
...
- 类似 Word 的位置：脚注位于其标记所在页面的底部、页脚之上，并带有分隔线。
- 自动、连续编号，会在每次插入、删除、粘贴和重新排版时更新。
- 随着脚注增多，页面正文会缩小——分页会考虑脚注所占空间。
- 支持富文本脚注内容，并使用你配置的扩展。
- 渲染后的脚注与编辑视图外观一致——排版、编号和间距都匹配，包括用作垂直间距的空段落。
- 无需额外配置即可与 DOCX 往返。

## 不应期待的行为

- **不会跨页面延续脚注。** 当某页脚注高度超过 `maxHeightRatio` 时，区域会被截断，而不会像 Word 那样在下一页继续显示溢出内容。
- **不渲染尾注。** DOCX 导入会暴露尾注数据供你自行处理，但 Pages 不会显示或管理尾注。
- **仅支持十进制编号。** 脚注在整个文档中连续编号为 `1, 2, 3…` —— 目前不支持每页重置以及其他格式（罗马数字、字母、符号）。
- **脚注中的表格不会导出。** 它们会在编辑器中显示，但 Word 的脚注格式只接受段落，因此在 DOCX 导出时表格会被丢弃。
...
| `footnotesHTML`      | `Record<string, string>`                     | 每个注释 id 对应的渲染后 HTML，与页面预览一致                 |
| `footnoteNumbers`    | `Record<string, number>`                     | 当前编号（注释 id → 从 1 开始的编号）                        |
| `footnotesEnabled`   | `boolean`                                    | 脚注是否已启用                                               |
| `editableFootnotes`  | `boolean`                                    | 是否已解锁编辑（只读——请使用 `setFootnotesEditable`）         |
| `activeEditorType`   | `'header' \| 'footer' \| 'footnotes' \| null` | 打开脚注编辑器时为 `'footnotes'`                              |
| `footnotesEditorOn`  | `Editor['on'] \| null`                       | 预绑定的脚注编辑器事件订阅函数                               |
| `footnotesEditorOff` | `Editor['off'] \| null`                      | 预绑定的脚注编辑器事件取消订阅函数                            |
