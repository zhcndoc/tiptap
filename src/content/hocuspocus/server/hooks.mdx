...
钩子需要返回一个 [Promise](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Promise)；最简单的方法是将函数标记为 `async`（自 v4 起需要 Node.js 22+）。通过这种方式，你可以执行诸如 API 请求、数据库查询、触发 webhook 等操作，或做任何你需要做的事情，以便将其集成到你的应用程序中。

自 v4 起，钩子负载使用 Web 标准的 `Request` 和 `Headers` 对象，而不是 Node.js 的 `IncomingMessage` 和 `IncomingHttpHeaders`。读取请求头时应使用 `requestHeaders.get('authorization')`，而不是 `requestHeaders['authorization']`。`onUpgrade` 和 `onRequest` 钩子是例外——它们仍然使用 Node.js 类型，因为它们是在 WebSocket 升级之前的 HTTP 层面上运行的。
...
| Hook                       | 说明                                       | 链接                                                  |
| -------------------------- |-------------------------------------------|-------------------------------------------------------|
| `beforeHandleMessage`      | 处理消息之前                              | [阅读更多](/hocuspocus/server/hooks#before-handle-message)      |
| `beforeHandleAwareness`    | 应用传入的 awareness 更新之前              | [阅读更多](/hocuspocus/server/hooks#before-handle-awareness)  |
| `onConnect`                | 连接建立时                                | [阅读更多](/hocuspocus/server/hooks#on-connect)                 |
| `connected`                | 连接建立后                                | [阅读更多](/hocuspocus/server/hooks#connected)                  |
| `onAuthenticate`           | 需要身份验证时                            | [阅读更多](/hocuspocus/server/hooks#on-authenticate)            |
| `onAwarenessUpdate`        | awareness 发生变化时                      | [阅读更多](/hocuspocus/server/hooks#on-awareness-update)        |
| `onLoadDocument`           | 创建新文档期间                            | [阅读更多](/hocuspocus/server/hooks#on-load-document)           |
| `afterLoadDocument`        | 文档创建后                                | [阅读更多](/hocuspocus/server/hooks#after-load-document)        |
...
| `onDestroy`                | 服务器即将被销毁时                        | [阅读更多](/hocuspocus/server/hooks#ondestroy)                 |
| `onConfigure`              | 服务器完成配置时                          | [阅读更多](/hocuspocus/server/hooks#on-configure)               |
| `onRequest`                | 有 HTTP 请求进入时                         | [阅读更多](/hocuspocus/server/hooks#on-request)                 |
| `onStoreDocument`          | 文档已更改时                              | [阅读更多](/hocuspocus/server/hooks#on-store-document)          |
| `onUpgrade`                | WebSocket 连接升级时                      | [阅读更多](/hocuspocus/server/hooks#on-upgrade)                 |
| `onStateless`              | 收到 Stateless 消息时                     | [阅读更多](/hocuspocus/server/hooks#on-stateless)               |
| `beforeBroadcastStateless` | 广播无状态消息之前                        | [阅读更多](/hocuspocus/server/hooks#before-broadcast-stateless) |
| `afterUnloadDocument`      | 文档关闭时                                | [阅读更多](/hocuspocus/server/hooks#after-unload-document)      |
...
`beforeHandleAwareness` 钩子会在服务器解码传入的 awareness 消息后、将其应用到文档的 awareness 状态之前调用。与在应用后观察 awareness 变化的 `onAwarenessUpdate` 不同，`beforeHandleAwareness` 允许你在消息进入文档的 awareness 或被广播给其他节点之前检查并重写更新内容。

该钩子接收按客户端分组、已解码的 `states`，其类型是一个可变的 `Map`，以 Yjs 的 `clientId` 为键。你可以原地修改这个 Map 及其中的状态对象，以便：

- 修改状态对象中的字段（例如，用服务器已验证的 `user` 覆盖客户端提供的值）。
- 使用 `states.delete(clientId)` 从广播中移除某个节点。
- 使用 `states.set(clientId, ...)` 添加或替换一个合成状态。

发送给其他节点的编码更新会反映钩子链中所有钩子执行后的 Map 状态。抛出异常会拒绝整个更新——不会被应用，也不会被广播。

多个扩展可以自然串联：每个扩展都会看到前一个扩展修改后的 Map，并可继续修改。扩展会先于配置层面的钩子执行。

**钩子负载**

传递给 `beforeHandleAwareness` 钩子的 `data` 具有以下属性：
...
const data = {
  awareness: Awareness,
  clientsCount: number,
  // 由 `onAuthenticate` 填充的连接上下文。若更新并非来自客户端连接，
  // 则为 `undefined`（例如，通过 DirectConnection 进行的服务器内部写入）。
  ...
  requestParameters: URLSearchParams,
  socketId: string,
  // 从传入更新中解码出的按客户端划分的 awareness 状态，按
  // Yjs clientId 为键。原地修改此 Map 以重写更新。
  states: Map<number, Record<string, any>>,
  // 将传递给 applyAwarenessUpdate 的 TransactionOrigin。
  // 使用 isTransactionOrigin() 来区分来源。
  transactionOrigin: TransactionOrigin,
  // 便捷快捷方式：当 transactionOrigin 是
  // ConnectionTransactionOrigin 时为 `origin.connection`，否则为 undefined。
  connection?: Connection,
}
```
...
const server = new Server({
  async beforeHandleAwareness({ context }) {
    if (context?.readOnly) {
      throw new Error('只读会话不允许 awareness 更新')
    }
  },
})
...
自 v4 起，该负载已简化。特定连接字段（`context`、`requestHeaders`、`requestParameters`、`socketId`）已被移除。请通过可选的 `connection` 访问它们（例如 `connection?.context`）。
...
在 v3 中，该负载还包含 `context`、`requestHeaders`、`requestParameters`、`socketId` 和 `transactionOrigin`。在 v4 中，这些字段已被移除或重命名——如有需要，请通过 `lastContext` 访问与连接相关的数据。
