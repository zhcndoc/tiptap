...
const server = new Server({
  extensions: [
    new Redis({
      ...
      // [可选] 等待另一个实例在加载文档时返回当前状态的最长时间（毫秒）。
      // 默认值为 1000。
...
当某个实例加载一个已经在另一个实例中打开的文档时，Redis 扩展会等待该实例发送其当前状态。这样可以确保新连接的客户端以及直接连接能够收到可能尚未持久化的更改。如果没有其他实例打开该文档，加载过程不会被延迟。

`awaitInitialSyncTimeout` 选项用于限制扩展等待的时间，单位为毫秒。其默认值为 `1000`。将其设置为 `0` 可禁用等待，改为使用 fire-and-forget 同步。
