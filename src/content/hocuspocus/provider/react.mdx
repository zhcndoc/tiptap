...
Peer 依赖：React 18 或 19，Yjs `^13.6.8`，以及与之匹配版本的 `@hocuspocus/provider`。
...
| `sessionAwareness` | `boolean`                                                   | 启用支持会话感知的多路复用，使多个具有相同文档名称的房间可以共享一个 WebSocket。仅在连接到 v4 服务器时设置为 `true`。默认 `false`。参见 [`sessionAwareness`](/hocuspocus/provider/configuration)。 |
| `flushDelay` | `false \| number`                                                 | 在短时间窗口内批量发送文档和 awareness 更新（单位：ms），以减少高强度编辑期间的 websocket 流量。默认 `false`。参见 [批量发送出站更新](/hocuspocus/provider/configuration#batching-outgoing-updates)。 |
...
返回当前 room 的 `HocuspocusProvider` 实例。当你需要直接访问 provider 时使用它——例如，将 Tiptap 的 `Collaboration` / `CollaborationCaret` 扩展连接到 provider 的 Y.Doc 和 awareness。
...
通过稳定订阅监听任意 [provider event](/hocuspocus/provider/events)（处理函数引用会在内部保持最新，因此你不需要对其进行 memoize）。
