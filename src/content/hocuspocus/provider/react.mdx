...
|------------|---------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------|
| `name`     | `string`                                                            | 文档名称。必需。                                                                                         |
| `document` | `Y.Doc`                                                             | 可选——传入你自己的 Y.Doc。若省略，则会自动为你创建一个。                                                  |
| `token`    | `string \| (() => string) \| (() => Promise<string>)`               | 发送到服务器用于身份验证的 JWT（或异步解析器）。                                                          |
| `sessionAwareness` | `boolean`                                                   | 启用会话感知的多路复用，使具有相同文档名称的多个 room 可以共享一个 WebSocket。仅在连接到 v4 服务器时设为 `true`。默认 `false`。参见 [`sessionAwareness`](/hocuspocus/provider/configuration)。 |
