...
让我给你一个例子：如果您将类似 `This is <strong>important</strong>` 的内容粘贴到 Tiptap 中，而没有处理 `strong` 标签的扩展，您将只会看到 `This is important` — 没有 strong 标签。

如果您想知道何时发生这种情况，可以在启用 `enableContentCheck` 选项后监听 [`contentError`](/editor/api/events#contenterror) 事件。

## 模式是什么样子的

当您完全使用提供的扩展时，其实不需要太在意模式。如果您正在构建自己的扩展，那么了解模式的工作方式可能会很有帮助。让我们看看典型 ProseMirror 编辑器最简单的模式：
