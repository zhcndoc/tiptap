...
| 命令                    | 描述                                                 |
| ----------------------- | ---------------------------------------------------- |
| `clearNodes()`          | 将节点规范化为一个简单段落。                         |
| `createParagraphNear()` | 创建一个附近的段落。                                 |
| `deleteNode()`          | 删除一个节点。                                       |
| `extendMarkRange()`     | 将文本选择范围扩展到当前标记。                       |
| `exitCode()`            | 退出代码块。                                         |
| `insertDefaultBlock()`  | 在某个位置插入默认块类型。                           |
| `joinBackward()`        | 向后合并两个节点。                                   |
| `joinForward()`         | 向前合并两个节点。                                   |
| `lift()`                | 移除现有的包裹。                                     |
| `liftEmptyBlock()`      | 如果块为空，则提升该块。                             |
| `newlineInCode()`       | 在代码中添加换行符。                                 |
| `resetAttributes()`     | 将某些节点或标记属性重置为默认值。                   |
| `setMark()`             | 使用新属性添加一个标记。                             |
| `setNode()`             | 用一个节点替换给定范围。                             |
| `splitBlock()`          | 从现有节点拆分出一个新节点。                         |
| `toggleMark()`          | 切换标记的启用和禁用状态。                           |
| `toggleNode()`          | 在一个节点与另一个节点之间切换。                     |
| `toggleWrap()`          | 用另一个节点包裹节点，或移除现有的包裹。             |
| `undoInputRule()`       | 撤销一条输入规则。                                   |
| `unsetAllMarks()`       | 移除当前选择中的所有标记。                           |
| `unsetMark()`           | 移除当前选择中的一个标记。                           |
