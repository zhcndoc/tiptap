...
`coreExtensionOptions` 属性用于配置 Tiptap 内置的核心扩展。

使用 `tabindex.value` 来设置编辑器元素上的 `tabindex` 属性。当 `value` 未定义时，可编辑编辑器默认使用 `tabindex="0"`，不可编辑编辑器不会获得 `tabindex` 属性。
...
`textDirection` 属性为编辑器中的所有内容设置文本方向。这对于阿拉伯语和希伯来语等从右到左（RTL）的语言，或双向文本内容非常有用。
...
您还可以使用 `setTextDirection` 和 `unsetTextDirection` 命令覆盖特定节点的方向。更多详情请参见 [命令文档](/editor/api/commands)。
...
是否启用扩展级别的事务分发。如果为 `false`，扩展将无法定义其自己的 `dispatchTransaction` 钩子。
