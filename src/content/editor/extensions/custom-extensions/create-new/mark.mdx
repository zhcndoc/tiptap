...
这段代码创建了一个名为 `HighlightMark` 的新标记扩展。它添加了一个 `addOptions` 方法来定义标记的选项，用户可以配置这些选项。它还添加了 `parseHTML` 和 `renderHTML` 方法，用于定义标记如何解析和渲染为 HTML。

安装方法与其他扩展相同，将其添加到 `extensions` 数组中即可。
