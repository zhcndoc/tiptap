...
| versions            | `array<Version>`       | 存储在历史记录中的版本数组。每个版本包含 `version`（数字）、`date`（数字）、可选的 `name`（字符串）和可选的 `meta`（对象）。 |
...
```js
// 导入 watchPreviewContent 函数（请参阅下面的详细信息）
...
要超越预览并以可视化方式比较不同版本，[比较快照](/collaboration/documents/snapshot-compare) 扩展提供了一种简单的方法来查看编辑器中任何两个版本之间的更改。
