...
<Requirements>
    ...
    <RequirementItem label="3. 认证">
        使用携带 `Documents:Api:All` 的令牌进行签名，并将其作为 `Authorization: Bearer <jwt>` 发送。请参阅 [身份验证](/authentication)。请将此令牌保留在服务器端。
    </RequirementItem>
</Requirements>
...
| `mode` | `string` | `default` | 更新策略：`default` 合并节点，`append` 仅追加，`node` 用负载替换匹配的节点，`attrs` 替换匹配节点的属性，`delete` 删除匹配的节点（无需负载）。参见 [模式](#modes)。 |
| `nodeAttributeName` | `string` | — | 将更新限制为具有此名称属性的节点。与 `nodeAttributeValue` 结合使用。 |
| `nodeAttributeValue` | `string` | — | 将更新限制为具有此值属性的节点。与 `nodeAttributeName` 结合使用。您可以传递多个值：`?nodeAttributeValue=a&nodeAttributeValue=b`。 |
| `checksum` | `string` | — | 仅当文档当前状态与此校验和匹配时才应用更新（在获取文档响应中作为 `x-FRAGMENTNAME-checksum` 返回）。否则返回 `409 Checksum mismatch`。 |
| `user` | `string` | — | 与此次更改关联的用户 ID。 |
| `skipVersioning` | `1` | — | 如果传递此参数，此次更改不会触发自动版本控制。 |
| `upsert` | `0` \| `1` | `0` | 当为 `1` 时，如果文档尚不存在，则创建文档，而不是返回 `404`。 |
| `fragment` | `string` | `default` | 要更新的 Yjs 片段。通过传递多个值，可以用相同内容更新多个片段（当 `format=json` 时）：`?fragment=a&fragment=b`。 |
| `multi` | `0` \| `1` | `0` | 仅与 `format=json` 一起使用。当为 `1` 时，请求体是一个以片段名为键的 JSON 对象，可在单个请求中用不同内容更新多个片段。仅与默认模式兼容。参见 [使用不同内容更新多个片段](#update-multiple-fragments-with-different-content)。 |
| `mergeAttributes` | `0` \| `1` | `0` | 仅与 `mode=attrs` 一起使用。当为 `1` 时，请求负载中未包含的属性将被保留。当为 `0`（默认）时，缺失的属性将被删除。 |
| `multiUpdates` | `true` | — | 仅与 `mode=attrs` 和 `format=json` 一起使用。允许在单个请求中更新多个具有不同属性值的节点。请求体必须是一个包含 `{ where, attrs }` 对象的 JSON 数组。参见 [多重更新](#multi-updates)。 |

更新成功后，服务器将返回 HTTP 状态 `204`。如果文档不存在，则返回 `404`（除非传递了 `upsert=1`，此时会创建文档）；如果负载无效或无法应用更新，则返回 `422`。
...
为了确保精确更新，尤其是针对特定节点的更改，建议使用 `nodeAttributeName` 和 `nodeAttributeValue` 参数。这些参数可以由 Tiptap 的 [UniqueID Extension](/editor/extensions/functionality/uniqueid) 或自定义实现生成。
请注意，这仅在使用 mode=default 时适用于顶层节点。
...
--data '{
  ...
  "content": [
    {
      "type": "paragraph",
      "content": [{ "type": "text", "text": "该段落将追加到末尾" }]
    }
  ]
}'
```
...
--data '{
  "type": "paragraph",
  "attrs": { "id": "abc-123" },
  "content": [{ "type": "text", "text": "这将替换整个匹配的节点" }]
}'
```
...
要在单个请求中使用**不同**内容更新多个片段，请传入 `multi=1`（配合 `format=json`），并发送一个以片段名称为键的 JSON 对象。每个值都是该片段完整的 Tiptap JSON 内容，并使用默认模式合并到对应片段中。

与 `?fragment=a&fragment=b`（将*相同*的载荷应用到每个列出的片段）不同，`multi=1` 允许你为每个片段发送不同内容。

`multi=1` 仅适用于默认模式。它不能与 `mode`、`nodeAttributeName`/`nodeAttributeValue`、`checksum` 或 `multiUpdates` 组合使用——否则会返回 `400`。它可以与 `user`、`skipVersioning` 和 `upsert` 组合使用。
...
--data '{
  "default": {
    "type": "doc",
    "content": [
      { "type": "paragraph", "content": [{ "type": "text", "text": "片段 default 中的文本" }] }
    ]
  },
  "otherFragment": {
    "type": "doc",
    "content": [
      { "type": "paragraph", "content": [{ "type": "text", "text": "片段 otherFragment 中的文本" }] }
    ]
  }
}'
```
...
--data '{
  ...
  "content": [
    {
      "type": "paragraph",
      "content": [{ "type": "text", "text": "仅在文档未更改时应用" }]
    }
  ]
}'
```
...
const secret = ''
const appId = ''; // 来自 Cloud 仪表板的文档服务器 ID;
...
If you only want to update a node’s attributes, you can use the `?mode=attrs` query parameter. This will update only the node’s attributes, not its content.
In this mode, the `nodeAttributeName` and `nodeAttributeValue` parameters apply to any node, not just top-level nodes.

By default, all attributes on matching nodes are replaced—attributes not included in the payload will be deleted. To preserve existing attributes and update only the attributes specified in the payload, pass `?mergeAttributes=1`.

Omitting the node filter (`nodeAttributeName`, `nodeAttributeValue`) will result in all nodes being updated.
