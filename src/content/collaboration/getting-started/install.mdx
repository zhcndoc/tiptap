...
- **name**: 作为同步的文档标识符。
- **appID**: 您的文档服务器 ID，在您的 [Cloud 仪表板](https://cloud.tiptap.dev/v2/configuration/document-server) 中找到，标签为 "Document server ID"。对于本地部署，将 `appID` 替换为 `baseUrl`。
- **token**: 使用来自您的 [Cloud 界面](https://cloud.tiptap.dev/v2/configuration/document-server) 的 JWT 进行测试，但在生产环境中请生成您自己的 JWT。
...
export default () => {
  ...
  useEffect(() => {
    const provider = new TiptapCollabProvider({
      ...
      appId: '7j9y6m10', // 来自 Cloud 仪表板的文档服务器 ID，本地部署请使用 `baseURL`
      token: 'notoken', // 您的 JWT
      document: doc,
    })
  }, [])
