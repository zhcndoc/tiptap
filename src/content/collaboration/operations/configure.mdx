...
| `webhook_include_fields`        | 设置为 `1` 会在 webhook 中包含自定义的 yjs 字段（参见 [webhooks](/collaboration/core-concepts/webhooks#custom-fields)）。                                                                                  |

## 通过 API 管理设置

协作平台提供了一个简单的 API 来管理这些设置。将 `:key` 替换为您要更新的设置键，将 `YOUR_APP_ID` 替换为您的文档服务器 ID（在 [Cloud 仪表板](https://cloud.tiptap.dev/v2/configuration/document-server) 中标记为"Document server ID"）。
