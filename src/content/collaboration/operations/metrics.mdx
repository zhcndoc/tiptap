...
将 `YOUR_APP_ID` 替换为您的文档服务器 ID，它在 [Cloud 仪表板](https://cloud.tiptap.dev/v2/configuration/document-server) 中标记为"Document server ID"。

### 认证
通过在 `Authorization` 请求头中包含您的 API 密钥来认证您的 API 请求。您可以在 Tiptap Cloud 仪表板的 [设置](https://cloud.tiptap.dev/v2/configuration/document-server) 中找到您的 API 密钥。
